...
// ── 胎元 IS DISPLAY ONLY. PROMPT M CORRECTION 4. ─────────────
// An earlier version of prompt M supplied a descriptive 胎元 sentence and said to
// ship it without waiting for Reyner. That instruction is REVERSED - it is now the
// prompt's own correction 4, ruled by Reyner 2026-08-21, which states the general
// precedence: a ruling recorded in the repo beats a Cowork build prompt, always. A
// prompt may correct a spec; it may not overrule him. The ruling being protected is
// `glossary.json` -> `pilar.conception._note`, 2026-08-07:
//
...
//
// The line the old prompt handed over is the exact thing that ruling removed once
// already: authored Indonesian living in code and in no glossary entry. Its group
// name was "Istana Konsepsi" too - the very string the ruling replaced with "Pilar
// Konsepsi", which is what GROUP_ORDER uses. Correction 4 fixed both in the prompt.
//
 ...
 */
/**
 * The PDF anchor name for an appendix entry. Build step 4 (correction 3).
 *
 * ── IT LIVES HERE BECAUSE TWO PLACES NEED IT AND MUST NOT DISAGREE ──
 * The document writes it as a react-pdf `id`, which becomes a named destination in
 * the produced PDF, and the verifier reads those destinations back to learn which
 * page each entry landed on. If the writer and the reader computed the name
 * separately, the cross-reference check would be comparing two derivations rather
 * than one artifact - the mistake `glyphProof` exists to avoid one file over.
 *
 * `${section}.${key}` is already the dedupe key above, so it is unique across the
 * whole appendix by construction. Sanitised because a PDF name string is bytes and
 * `key` is a hanzi branch for every Shio entry: `shio.巳` becomes `shio.u5df3`.
 * Reversibility does not matter - only that it is deterministic and collision-free,
 * and `assertAnchorsUnique` checks the second part rather than assuming it.
 */
export function anchorId(entry) {
  const safe = (s) => String(s).replace(/[^A-Za-z0-9_-]/g,
    (c) => `u${c.codePointAt(0).toString(16)}`);
  return `${safe(entry.section)}.${safe(entry.key)}`;
}

/** Every anchor in the appendix, in document order. */
export function anchorIds(appendix) {
  return appendix.groups.flatMap((g) => g.entries.map((e) => anchorId(e)));
}

/**
 * Refuses a chart whose sanitised anchors collide. Two entries sharing an anchor
 * would share a destination, and the cross-reference for one of them would point at
 * the other's page while every check still passed - a wrong reference that verifies.
 *
 * @throws {Error}
 */
export function assertAnchorsUnique(appendix) {
  const ids = anchorIds(appendix);
  const dupes = ids.filter((id, i) => ids.indexOf(id) !== i);
  if (dupes.length) {
    throw new Error(`appendix: anchor collision after sanitising - ${[...new Set(dupes)].join(', ')}`);
  }
  return ids;
}

/**
 * A condition's label, or null.
 *
 * ── THIS IS NOT A REVERSAL OF CORRECTION 1. READ BOTH. ────
 * Correction 1 forbids naming a condition, and names the defect it was written
 * against: `Kayu yang Hilang`, "the same defect as printing 'Missing Wood' - both
 * are naming a thing she does not carry." That reasoning is intact and this
 * satisfies it.
 *
 * RULED BY REYNER 2026-09-22 (P2 markup row A6): the format is `Tanpa [Elemen]` /
 * `Dominan [Elemen]`. `Tanpa Kayu` does not name a thing she does not carry - it
 * names the ABSENCE, which is what the row is about, and that is the difference
 * between it and `Kayu yang Hilang`. So the two rulings agree: no invented noun for
 * a mechanic she lacks, and a plain label for the lack itself.
 *
 * THE ELEMENT COMES FROM THE FACT, NOT FROM THE GLOSSARY KEY, and that is not a
 * detail. `elemen_hilang`'s keys are elements (`木 火 土 金 水`) but
 * `elemen_dominan`'s are RELATIONS (`same feeds drains is_controlled controls`) -
 * its `controls` entry is `name_en: "Dominant Officer"`, a Ten God and not an
 * element. Deriving the word from the key would have produced `Dominan Controls`.
 * `fact.provenance.element` already carries the ruled Indonesian word (`Kayu`,
 * `Air`), which is why this takes the fact.
 *
 * Anything else nameless stays nameless: a condition with no element is not covered
 * by the ruling and must not be given a label this function invented.
 *
 * @param {Object} fact a semantic fact with `label === null`
 * @returns {string|null}
 */
function conditionName(fact) {
  const element = fact?.provenance?.element;
  if (!element) return null;
  if (fact.provenance.kind === 'element_absent') return `Tanpa ${element}`;
  if (fact.provenance.kind === 'element_dominant') return `Dominan ${element}`;
  return null;
}
...
export function buildAppendix({ chart, semanticJson }) {
  ...
  for (const fact of semanticJson.facts || []) {
    ...
    push(src.group, {
      key: src.key,
      section: src.section,
      name: condition ? conditionName(fact) : (entry.name_id ?? fact.label ?? null),
  ...
  // ── A ROW WITH NO MEANING DOES NOT PRINT, AND NEITHER DOES ITS GROUP ──
  // Markup row A7, ruled ok 2026-09-22. `pilar/conception` carries no
  // `label_meaning` by Reyner's standing ruling of 2026-08-07, and the appendix
  // printed it anyway: a group heading `Pilar Konsepsi`, then a row labelled `Pilar
  // Konsepsi`, then nothing. An empty cell in a document she paid Rp 19.000 for.
  //
  // OMITTED RATHER THAN FILLED, which is the same choice correction 2 made about
  // names. Prompt M's correction 4 records a session being told to ship a
  // Cowork-drafted meaning for 胎元 and that is exactly what must not happen here.
  // 胎元 still reaches her - the CHART PAGE prints it with its animal - so what is
  // removed is a legend entry that explained nothing, not the fact itself.
  //
  // IF REYNER EVER RULES A MEANING, THE ROW COMES BACK WITH NO CODE CHANGE: this
  // filters on emptiness, not on the key.
  const groups = GROUP_ORDER
    .map((group) => ({
      group,
      entries: byGroup.get(group).filter((e) => String(e.meaning || '').trim() !== ''),
    }))
