...
import {
  PDF_STYLES as s, chartPage, appendixPages, referenceRows, chartPageFoot, refRow,
} from './document.js';
...
export function factRows(semanticJson) {
  ...
  // The palace frame, and every relation it found. Its own cell is nameless, so it
  // reaches the table as a meaning with no term - correction 1's shape again.
  const frame = factOf(semanticJson, 'p2_palace_frame');
  for (const key of frame?.provenance?.variants || []) {
    if (key === p2?.provenance?.variant) continue; // already its own row
    add(key, '', '');
  }
  ...
  if (supplies.length > 0) {
    for (const sup of supplies) {
      const element = elementId(sup.element) ?? '';
      // `from` is 'a' or 'b'. The element appears in the GIVER's column, so the
      // direction is readable without a sentence saying it.
      add('p3_supplies', sup.from === 'a' ? element : '', sup.from === 'b' ? element : '');
    }
  } else if (p3) {
    add('p3_no_supply', '', '');
  }
...
function coverPage({ semanticJson, pair }) {
  ...
  return E(Page, { size: 'A4', style: s.page },
    // A first, B second - ENGINE ORDER, which is the reader's order. `core.a` is
    // the buyer in every pair payload.
 ...
 */
function factsPage({
  semanticJson, rendered, rows, refs, pageMap,
}) {
...
export function pairDocument({
  ...
  // The facts page's reference rows: one per NAMED row in the table, deduped, in
  // table order. Derived from the rows themselves rather than from `appendix.compat`
  // so the page references what it actually printed.
  const byKey = new Map(appendix.compat.map((e) => [e.key, e]));
  const factRefs = [];
  for (const r of rows) {
    const entry = r.anchorKey ? byKey.get(r.anchorKey) : null;
    if (entry && !factRefs.includes(entry)) factRefs.push(entry);
  }

  const chartFor = (chart, heading, refs) => chartPage({
    chart,
  ...
  chartFor(chartA, PASANGAN_COPY.pdf_chart_a_heading, appendix.chartA),
  chartFor(chartB, PASANGAN_COPY.pdf_chart_b_heading, appendix.chartB),
  factsPage({
    semanticJson, rendered, rows, refs: factRefs, pageMap,
  }),
