...
import { pairLine, GENDER_WORDS } from '../site/birthSummary.js';
...
function coverPage({ semanticJson, pair, rendered }) {
  const a = semanticJson.core?.a || {};
  const b = semanticJson.core?.b || {};
  // ── A2: THE REPORT HEADER'S OWN LINE. Ruled ok 2026-09-22. ──
  // It printed `1989-09-13 - female`: ISO dates and ENGLISH gender words in an
  // Indonesian document. `pairLine` is the function the web report's header already
  // calls - "Perempuan, 13 September 1989 dan Laki-laki, 4 Maret 1990" - so the
  // page and the document name the pair identically, by construction.
  const births = pairLine(pair, { genderWords: GENDER_WORDS });
  return E(Page, { size: 'A4', style: s.page },
    ...
    ...(births ? [E(Text, { style: s.coverBirth }, births)] : []),
  ...
  return E(Page, { size: 'A4', style: [s.page, s.tablePage], wrap: true },
    ...
    ...rows.flatMap((r, i) => {
      ...
      return [
        ...
        ...(pairLevel(r) ? [E(View, { key: `p${i}`, style: s.factsGroup, wrap: false },
...
export function pairDocument({
  ...
  coverPage({ semanticJson, pair, rendered }),
