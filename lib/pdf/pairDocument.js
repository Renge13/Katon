...
function factsPage({ semanticJson, rendered, rows }) {
  ...
  return E(Page, { size: 'A4', style: s.page, wrap: true },
    ...
    ...rows.flatMap((r, i) => [
      ...
      // reframe (ruled 2026-09-22), and drawing it as a row with two blank cells is
      // the same defect C4 names for the verdict row one section down.
      ...(r.note ? [E(Text, { key: `n${i}`, style: s.factsNote }, r.meaning)] : []),
      // Kept together so a term never sits at the foot of a page with its columns
      // and meaning on the next - markup row A9, the same rule the appendix gets.
      ...(r.note ? [] : [E(View, { key: `r${i}`, style: col, wrap: false },
      ...
      // A frame row carries no meaning cell now, so it prints none rather than
      // printing an empty Text that still takes vertical space.
      ...(!r.note && String(r.meaning || '').trim()
