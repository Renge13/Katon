...
//
// ── THE SIX GROUPS, AND THE ONE THAT IS DELIBERATELY ABSENT ──
// Y-3 enumerates the per-chart groups: Aspek, Bintang, Elemen dan Kekuatan, Pilar,
// Shio, Pilar Konsepsi. `Relasi Cabang` - a SINGLE chart's own branch relations - is
// not among them and its absence is the ruling rather than an oversight. The pair's
// P2 block is about branch relations BETWEEN the two charts, and printing one
// person's internal 冲 in the same legend as `Kursi Berbenturan` invites the reader
// to read the first as the second. If Reyner wants it in, it is one entry in
// `PER_CHART_GROUPS`.
//
 ...
 */
export const PER_CHART_GROUPS = GROUP_ORDER.filter((g) => g !== 'Relasi Cabang');
...
export function buildPairAppendix({ chartA, chartB, semanticJson }) {
  ...
  for (const fact of semanticJson.facts || []) {
    for (const key of variantKeysFor(fact)) {
      ...
      if (!cell) continue;
      // CORRECTION 1, and it arrives here for free rather than as a special case: a
      // compat cell with no `name_id` - `p2_reframe`, `p2_palace_frame` - is a
      // meaning without a name, which is the same shape as a mirror CONDITION. It
      // gets its ruled meaning, no heading, and no line in any reference list.
      const unnamed = !cell.name_id;
