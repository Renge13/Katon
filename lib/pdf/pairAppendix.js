...
export function buildPairAppendix({ chartA, chartB, semanticJson }) {
  ...
  const perChart = (chart, groups = PER_CHART_GROUPS) => {
    const kept = [];
  ...
  // ── THE PARTNER CONTRIBUTES SHIO ONLY. R2 = A, 2026-09-24. ───
  // (Round-1 markup, Cowork by Reyner's delegation.) Every per-chart glossary entry
  // is the MIRROR's second-person text, written to the chart's owner - so the
  // partner's terms were printed to the BUYER as facts about her: `Seimbang` "Baganmu
  // berdiri di titik tengah yang stabil" one row under her own `Lemah`, `Dominan
  // Tanah` under her `Dominan Air`. False statements about the reader, the same class
  // as C2. Option B (a "Dari bagan dia" group) needs third-person meanings Reyner has
  // not written, so it is not built. SHIO STAYS, from both charts: its text names the
  // animal and says nothing to "kamu", and the partner's branches print in the
  // facts table beside their animals, so the legend has to explain them.
  const b = perChart(chartB, ['Shio']);
