...
export function latinText(pdf) {
  // ── NOW THE DECODED PAGE TEXT, IN PAGE ORDER (2026-09-23) ──
  // It concatenated `textByFont` - every run grouped BY FONT - and dropped CID runs,
  // which was the Latin text while Helvetica was the only Latin face. With the
  // headings in embedded Spectral (AB §4) that reading lost every heading. The page
  // text decodes CID runs through their ToUnicode CMaps (`pageFontDecoders`), so it
  // is the same words plus the ones it could not see, and it is in reading order
  // rather than font order. The NUL filter still applies to anything undecodable.
...
export function pageTexts(pdf) {
  ...
  return pageObjectOrder(pdf).map((pageObj) => {
    ...
    return contentStreamRuns(body, pageFontDecoders(objects, pageObj))
...
/**
 * One ToUnicode CMap, as CID -> text.
 *
 * THE SAME THREE FORMS `drawnCodePoints` reads, with the same discipline: the ARRAY
 * form of bfrange first and consumed, then the scalar form over what is left, and a
 * range whose shape is not recognised is SKIPPED rather than guessed at. A decoder
 * that guesses is the permissive failure that once reported 申 drawn in a document
 * that never drew it.
 *
 * @param {string} txt the inflated CMap stream
 * @returns {Map<number, string>}
 */
function parseToUnicode(txt) {
  const map = new Map();
  const str = (hex) => String.fromCodePoint(...utf16beCodePoints(hex));
  for (const block of txt.match(/beginbfchar([\s\S]*?)endbfchar/g) || []) {
    for (const m of block.matchAll(/<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>/g)) {
      map.set(parseInt(m[1], 16), str(m[2]));
    }
  }
  for (const block of txt.match(/beginbfrange([\s\S]*?)endbfrange/g) || []) {
    let rest = block;
    for (const a of block.matchAll(/<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>\s*\[([^\]]*)\]/g)) {
      const lo = parseInt(a[1], 16);
      const dsts = a[3].match(/<([0-9a-fA-F]*)>/g) || [];
      dsts.forEach((hex, i) => map.set(lo + i, str(hex.slice(1, -1))));
      rest = rest.split(a[0]).join(' ');
    }
    for (const sm of rest.matchAll(/<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>/g)) {
      const lo = parseInt(sm[1], 16);
      const hi = parseInt(sm[2], 16);
      const dst = utf16beCodePoints(sm[3]);
      if (dst.length !== 1 || hi < lo || hi - lo > 0xffff) continue;
      for (let i = 0; i <= hi - lo; i += 1) map.set(lo + i, String.fromCodePoint(dst[0] + i));
    }
  }
  return map;
}
...
/**
 * A page's font resources, as `F1` -> hex-run decoder.
 *
 * ── WHY THIS EXISTS (AB §4, 2026-09-23) ────────────────────
 * Until the headings went into Spectral, the only embedded face was the hanzi one,
 * and dropping its CID runs was a fair trade: the page text was the Latin text. An
 * embedded LATIN face breaks that trade - every serif heading vanished from
 * `pageTexts`, `build.js` verify 2 threw "no page carries the appendix heading", and
 * every test that finds a page by its heading went blind. So a Type0 font with a
 * ToUnicode CMap is DECODED, two bytes per CID, which is exactly what a viewer's
 * copy-paste does. A simple font (Helvetica, WinAnsi) keeps the latin1 reading.
 *
 * A CID the CMap does not name decodes to NOTHING rather than to a guess, and a
 * Type0 font with no ToUnicode keeps the old behaviour (latin1, then dropped by the
 * NUL filter) - both are the conservative direction.
 *
 * @returns {Map<string, (hex: string) => string>}
 */
function pageFontDecoders(objects, pageObj) {
  const out = new Map();
  const head = objects.get(pageObj)?.head || '';
  const resRef = /\/Resources\s+(\d+)\s+\d+\s+R/.exec(head);
  const res = resRef ? (objects.get(resRef[1])?.head || '') : head;
  const fontDict = /\/Font\s*<<([\s\S]*?)>>/.exec(res);
  if (!fontDict) return out;
  for (const f of fontDict[1].matchAll(/\/(F\d+)\s+(\d+)\s+\d+\s+R/g)) {
    const font = objects.get(f[2])?.head || '';
    if (!/\/Subtype\s*\/Type0/.test(font)) continue;
    const tu = /\/ToUnicode\s+(\d+)\s+\d+\s+R/.exec(font);
    const cmap = tu ? objects.get(tu[1])?.stream : null;
    if (!cmap) continue;
    const map = parseToUnicode(cmap);
    out.set(f[1], (hex) => {
      let s = '';
      for (let i = 0; i + 4 <= hex.length; i += 4) s += map.get(parseInt(hex.slice(i, i + 4), 16)) ?? '';
      return s;
    });
  }
  return out;
 ...
 *
 * @param {string} txt
 * @param {Map<string, Function>} [decoders] from `pageFontDecoders`
 */
function contentStreamRuns(txt, decoders = new Map()) {
  const out = [];
  const re = /\/(F\d+)\s+[\d.]+\s+Tf|\[([^\]]*)\]\s*TJ|\(((?:[^()\\]|\\.)*)\)\s*Tj/g;
  let m;
  let run = '';
  ...
  while ((m = re.exec(txt)) !== null) {
    ...
    if (m[1]) {
      ...
      decode = decoders.get(m[1]) || null;
      continue;
    }
    if (m[2] !== undefined) {
      for (const hex of m[2].match(/<([0-9a-fA-F]*)>/g) || []) {
        const h = hex.slice(1, -1);
        run += decode ? decode(h) : Buffer.from(h, 'hex').toString('latin1');
      }
    } else if (m[3] !== undefined) {
      run += m[3].replace(/\\(.)/g, '$1');
    }
...
// ============================================================
// WHERE TEXT IS ACTUALLY DRAWN (added 2026-09-14, Reyner ruling 3)
// ============================================================
const TRANSLATE = /^1 0 0 1 ([-\d.]+) ([-\d.]+) cm$/;
const TF = /^\/(F\d+) ([\d.]+) Tf$/;

export function textBoxes(pdf) {
  const objects = pdfObjects(pdf);
  return pageObjectOrder(pdf).map((pageObj) => {
    const c = /\/Contents\s+(\d+)\s+\d+\s+R/.exec(objects.get(pageObj)?.head || '');
    const body = c ? (objects.get(c[1])?.stream || '') : '';
    // Decoded through the page's own fonts, like `pageTexts`: a CID run read as
    // latin1 was being returned here as glyph-id bytes (measured 2026-09-23 on a
    // probe page - U+0001..U+000E where "Istilah dalam Bacaanmu" was drawn).
    const decoders = pageFontDecoders(objects, pageObj);
    const runs = [];
    let x = 0;
    let y = 0;
    let size = 0;
    ...
    for (const line of body.split(/\r?\n/)) {
      const t = line.trim();
      if (t === 'q') { stack.push([x, y]); continue; }
      if (t === 'Q') { [x, y] = stack.pop() || [0, 0]; continue; }
      const tr = TRANSLATE.exec(t);
      if (tr) { x += parseFloat(tr[1]); y += parseFloat(tr[2]); continue; }
      const tf = TF.exec(t);
      if (tf) { font = tf[1]; size = parseFloat(tf[2]); continue; }
      let run = null;
      const tj = /^\[(.*)\]\s*TJ$/.exec(t);
      if (tj) {
        run = '';
        const decode = decoders.get(font);
        for (const hex of tj[1].match(/<([0-9a-fA-F]*)>/g) || []) {
          const h = hex.slice(1, -1);
          run += decode ? decode(h) : Buffer.from(h, 'hex').toString('latin1');
        }
      } else {
      ...
      if (run !== null && run.replace(/\0/g, '').length) {
        runs.push({ x, y, size, font, text: run.replace(/\0/g, '') });
      }
    }
    return runs;
  });
...
/**
 * Runs whose boxes collide vertically, in the same column.
 *
 * `y` is top-down (the outer flip and the per-run `Tm` cancel), so a run's box runs
 * from `y` down by its own font size. Two runs in the same column collide when the
 * later one starts before the earlier one has finished.
 */
export const GLYPH_ASCENT = 0.78;
export const GLYPH_DESCENT = 0.30;

export function collisions(runs, {
  ascent = GLYPH_ASCENT, descent = GLYPH_DESCENT,
} = {}) {
  const out = [];
  ...
  for (let i = 0; i < sorted.length; i += 1) {
    for (let j = i + 1; j < sorted.length; j += 1) {
      ...
      const b = sorted[j];
      const gap = b.y - a.y;
      // The upper run's ink reaches `descent * size` below its baseline; the lower
      // run's reaches `ascent * size` above its own. They clear when the baselines
      // are at least the sum apart.
      const need = a.size * descent + b.size * ascent;
      if (gap >= a.size * descent + 60 * ascent) break; // sorted: no later run is closer
      if (gap >= need) continue;
      // gap 0 IS ONE LINE, not a collision. A wrapped line is emitted as several
      // runs at the same y, and the width estimate below is too coarse to separate
      // them - it reported `21 ` over `istilah, semuanya ...` on the mirror's
      // appendix, which is one sentence. Same baseline means side by side.
      if (gap === 0) continue;
      // SAME COLUMN. Two cells side by side share a y and must not be reported.
      const aR = a.x + a.text.length * a.size * 0.6;
      const bR = b.x + b.text.length * b.size * 0.6;
      if (Math.min(aR, bR) <= Math.max(a.x, b.x)) continue;
      out.push({
        a, b, gap: Number(gap.toFixed(1)), need: Number(need.toFixed(1)),
      });
    }
  }
  return out;
