...
export const PDF_STYLES = StyleSheet.create({
  ...
  factsGroup: { marginBottom: 10 },
  factsDesc: {
    fontSize: 9.5, lineHeight: 1.5, color: '#666666', marginTop: 3,
  },
 ...
 */
function coverPage({ chart, semanticJson, rendered }) {
  const core = semanticJson.core || {};
  const sc = semanticJson.chart || {};
  // ── A2: THE RULED BIRTH FORM, not ISO. Ruled ok 2026-09-22. ──
  // `13 Sep 1989, 09.00` - the stepper's own summary, through the one function
  // that formats it. The mirror's chart carries no gender, and `birthSummary`
  // omits an absent gender silently by design.
  const birth = birthSummary(
    { date: chart.birthDate, time: chart.birthTime, gender: chart.gender },
...
export function elementBars(presence) {
  ...
  return rows.map(([name, value]) => {
    ...
    return E(View, { key: `bar-${name}`, style: s.barRow },
      ...
      E(Text, { style: s.barValue }, String(value)));
  });
}
...
export function completeEdition({
  ...
  coverPage({ chart, semanticJson, rendered }),
