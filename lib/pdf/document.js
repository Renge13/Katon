...
export const PDF_STYLES = StyleSheet.create({
  ...
  penutup: { marginTop: 14, fontSize: 12, lineHeight: 1.25 },
...
export function chartPage({
  ...
  // ── TWO PARAMETERS FOR THE COMPAT DOCUMENT (prompt Y-3 commit 3) ──
  // Both default to exactly what the mirror did before they existed, so the mirror
  // document is unmoved - asserted by `tests/fixtures/pdf-chart1-pages.json` rather
  // than claimed.
  //
  // `heading` because the compat document prints TWO chart pages and their headings
  // are ruled copy slots, not a constant this file can know.
  //
  // `refEntries` because the compat appendix is ONE legend for two charts, so the
  // rows this page must print are a subset of it - chart A's terms on chart A's
  // page - and deriving them from the whole appendix would print chart B's Shio on
  // chart A's page. The mirror has one chart and passes nothing.
  heading = 'Bagan Kelahiran', refEntries = null,
}) {
  ...
  return E(Page, { size: 'A4', style: s.page, wrap: true },
    ...
    ...presence.map(([name, value]) => E(Text, { key: name, style: s.entryMeaning },
      `${name}: ${value}`)),
    ...(refEntries ? referenceRows(refEntries, pageMap) : []),
    ...(!refEntries && appendix ? referenceList(appendix, pageMap) : []),
...
function referenceList(appendix, pageMap) {
  ...
  return referenceRows(rows, pageMap);
}

/**
 * The rows themselves, for a list of entries that is already chosen.
 *
 * SPLIT OUT FOR THE COMPAT DOCUMENT (prompt Y-3 commit 3), which has THREE of these
 * lists over ONE appendix - chart A's terms, chart B's terms, the pair's own - so
 * the choosing is the caller's and the drawing is here. The mirror passes the same
 * list it always did, which is why `tests/fixtures/pdf-chart1-pages.json` does not
 * move. One implementation of a row, shared with `build.js` verify 3 through
 * `refRow`, is the whole point.
 */
export function referenceRows(rows, pageMap) {
