...
export function textRuns(html, baseGround) {
  ...
  const stack = [{ color: null, opacity: 1, ground: baseGround, hidden: false, raw: false }];
  let m;
  TAG.lastIndex = 0;
  while ((m = TAG.exec(html)) !== null) {
    ...
    if (text !== undefined) {
      ...
      //
      // `raw` is different and is NOT an exemption: the contents of <style> and
      // <script> are not text on the card at all. The card gained a <style> when
      // the hanzi @font-face started travelling with the object (2026-08-17), and
      // without this the audit read a CSS rule as a text run, found no colour on
      // it, and reported UNRESOLVED COLOUR on every token. An element whose
      // children are never painted has no contrast to measure.
      if (t && !top.hidden && !top.raw) {
    ...
    const frame = {
      ...
      raw: parent.raw || tag === 'style' || tag === 'script',
    };
