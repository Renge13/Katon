 ...
 *
 * THE BAND STEPS AWAY FROM THE INK — toward white under a dark ink, toward black
 * under a light one — so it always has MORE contrast than the flat field. That
 * makes the field the worst case on either card, which is what lets one audit
 * cover both. The first version tinted toward the ink and silently cost contrast
 * on the tokens with none to spare.
 */
export function surfaces(token, bandTint) {
  const away = luminance(token.ink) < 0.5 ? '#ffffff' : '#000000';
  return { field: token.field, band: composite(away, token.field, bandTint) };
}
