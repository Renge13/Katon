 ...
 *
 * `docs/content/sharecard-tokens-measure.mjs` already sets the bar as the locked
 ...
 *
 * THE FLOOR IS DERIVED FROM THE `approved: true` ROWS, never written down, so
 * approving one of the five open tokens moves the bar on its own with no edit
 * here. Measured 2026-08-14: the locked five are 3.31 / 3.43 / 3.45 / 5.40 /
 * 5.69, so the floor is 3.31 — Matahari, sitting exactly on it.
 *
 * @param {Record<string, {field: string, ink: string, accent: string, approved: boolean}>} tokens
 */
export function accentAudit(tokens) {
  const approved = Object.values(tokens).filter((t) => t.approved);
  const floor = Math.min(...approved.map((t) => contrast(t.accent, t.field)));
  const rows = Object.entries(tokens).map(([stem, t]) => {
    ...
    return {
      ...
      ratio,
      approved: t.approved,
      // The epsilon is not decoration: the token that DEFINES the floor sits
      // exactly on it, and a bare `<` reports Matahari as failing its own bar.
      under: ratio < floor - 1e-9,
    };
  });
  return { floor, rows, under: rows.filter((r) => r.under).map((r) => r.stem) };
}
