...
//             `payment_closed`; since 2026-09-23 no paid CTA renders at all
//             (`checkoutOpen()` below), and /kompatibilitas answers an old link
//             with its sales-closed state. **Existing paid pairs and
...
/**
 * Every reason `paymentFenceReason()` can return. EXPORTED so a client that
 * receives one from `POST /api/pay` can tell "sales are closed" from "something
 * broke" without holding its own list of the strings this module emits.
 */
export const FENCE_REFUSALS = Object.freeze(['payment_closed', 'doku_client_id_unset', 'doku_secret_key_unset']);
...
/**
 * Can a reader start a checkout right now?
 *
 * THE ONE ANSWER EVERY PAID CTA RENDERS FROM. RULED 2026-09-23 (Reyner): while the
 * fence is closed, no paid entry point renders at all. It is `paymentFenceReason()`
 * returning null and nothing else, so the page hides the CTA in exactly the cases
 * `POST /api/pay` would refuse it - a misconfigured DOKU included, which would 503
 * and hang the button the same way `closed` did in production.
 *
 * Called by the SERVER pages and passed down as a prop. A component never reads the
 * env itself, and never re-derives this from `paymentsProvider()`: a second copy of
 * the rule is how a page comes to offer what the route refuses.
 *
 * @returns {boolean}
 */
export function checkoutOpen() {
  return paymentFenceReason() === null;
}
