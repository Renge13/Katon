...
// ============================================================
// Prompt V §3. THE ONLY PLACE `paid` FLIPS FOR A REAL PAYMENT (CLAUDE.md rule 18),
// and it flips it through `settlePair` / `markReadingPaid`, the same single door the
// mock walk uses. Nothing here is a second way to become paid.
//
...
// the signature already gives. `checkStatus` exists for the ops script.
//
...
import { getReading, markReadingPaid } from '../readingStore.js';
...
export async function handleDokuNotification(request) {
  ...
  // exactly as the old webhook did it, and fail-closed on a null sku: a row with no
  // sku has no price to check against, and "no price" must not mean "any price".
  const amountOk = settledAmount === null ? true : amountMatchesSku(settledAmount, row.sku);
  if (statusPaid && !amountOk) {
    console.error(`[doku] amount rejected for reading ${rowId}: sku=${row.sku ?? 'null'}`);
    return json({ received: true });
  }
  if (!statusPaid) {
    console.log(`[doku] reading ${rowId}: not paid (${body?.transaction?.status ?? 'no status'})`);
    return json({ received: true });
  }

  const transitioned = await markReadingPaid(rowId, new Date().toISOString());
  // THE COUNTER FLIPS WHERE `paid` FLIPS AND NOWHERE ELSE, so a DOKU retry of a
  // settled payment does not count a second purchase.
  if (transitioned) await recordEvent(rowId, 'purchase_confirmed', { sku: row.sku ?? null });
  console.log(`[doku] reading ${rowId}: paid=${transitioned}`);
  return json({ received: true });
}
