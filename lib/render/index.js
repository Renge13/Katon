...
import { withEngineOpening } from './pairOpening.js';
import { scrubInternal } from './payload.js';
import { readCache, writeCache } from './cache.js';
import { STAGE6_VERSION, serveFenceReason } from './fence.js';
import { validateRenderingV2, v2Directive } from '../validate/v2.js';
import { judgeRendering } from '../validate/judge.js';
...
async function renderOnce(semanticJson, {
  ...
  // VOICE v2 gets room for a longer reading: the writer is no longer told to be
  // brief, and a truncated JSON is a malformed draft. A transport setting only.
  const config = {
    ...GENERATION,
    ...(semanticJson.voice === 'v2' ? { maxOutputTokens: 8192 } : {}),
    ...generation,
    fetchImpl,
  };
  ...
  // VOICE v2 pairs supply BOTH people's mirror facts (E2, `semanticJson.mirror`)
  // and the compat prompt asks for each person before the relationship, so a block
  // about one person cites that person's mirror facts. Without their ids here the
  // shape check refused every such draft as "cites unknown fact" and both measured
  // v2 pairs floored (2026-09-24, reports/voice-v2 first run). v1 has no `mirror`
  // and its list is unchanged. An id both people carry (spouse_palace) is still one
  // SUPPLIED fact, which is the only thing this check asks.
  const knownFactIds = [
    ...(semanticJson.facts || []),
  ...
  // VOICE v2: ONE regeneration (spec §4: "a render that fails after its
  // regeneration is served as the floor"). v1 keeps REGENERATION_BUDGET.
  const isV2 = semanticJson.voice === 'v2';
  let regenerationsLeft = isV2 ? Math.min(1, validationRetries) : validationRetries;
  // Judge findings of REJECTED v2 drafts, kept so a regeneration can be argued with.
  ...
  for (const provider of chain) {
    ...
    for (;;) {
      ...
      try {
        ...
        raw = await provider.call(`${loadPrompt(semanticJson.kind, semanticJson.voice)}${directive}`, payload, {
      ...
      // ── THE OPENING IS THE ENGINE'S, AND IT GOES IN BEFORE THE GATE ──
      // A pair reading's first block is a sentence Reyner ruled; the model is told
      // not to write one (docs/content/compat-renderer-prompt.txt). This puts the
      // ruled text in whether the model obeyed or not, so what Stage 6 validates
      // and what `persistRendered` caches are the same object. Mirror: no-op.
      const { rendered: forGate, hadOwnOpening } = withEngineOpening(parsed, semanticJson);
      // LOG ONLY. The model was asked not to write an opening; this counts the
      // times it did anyway, on an attempt record the QA tape already reads. It
      // rejects nothing - the block is dropped either way, so a count is the whole
      // remedy and a gate here would only turn a fixed defect into a floor.
      const wroteAnyway = hadOwnOpening ? { p0_model_wrote_anyway: true } : {};

      // ── Stage 6 ──────────────────────────────────────────
      const gate = isV2
        ? validateRenderingV2(forGate, semanticJson, { provider: provider.name })
        : validateRendering(forGate, semanticJson, { provider: provider.name });

      // ── VOICE v2: THE JUDGE, J1-J4 (spec §4b); J1 GATES SINCE 1.33.0 ──
      // (The paragraph below is the 1.29.0 record; the J1 block after the call is
      // what changed it.)
      // Ruled by Reyner 2026-09-24 after the first calibration failed
      // (docs/qa/2026-09-24-voice-v2-judge-calibration.md): the judge RUNS and its
      // findings are stored with the render, but it NEVER REJECTS. D1-D4 alone
      // gate v2. Every judge finding is recorded at `flag`; the severity the spec
      // would have given it is kept as `judge_severity`, so "what would a gating
      // judge have done" stays answerable from the stored review. A judge that
      // cannot run is recorded and changes nothing. Only after D1-D4 pass: a draft
      // already rejected is not worth a second model call.
      let judged = null;
      if (isV2 && gate.ok) {
        try {
          judged = await judgeRendering(gate.normalized, payload, { fetchImpl: config.fetchImpl });
        } catch (err) {
          judged = {
            findings: [{ check: 'v2.judge_unavailable', severity: 'hard', message: err.message }],
            malformed: [], usage: null, unavailable: true,
          };
        }
        // ── J1 IS HARD SINCE 1.33.0 (round 3, Reyner 2026-09-24) ──
        // J1 passed its calibration (docs/qa/2026-09-25-voice-v2-j1-calibration.md:
        // 15/15 seeded inventions caught, 0 J1 false positives on the clean set,
        // 3 runs), so a J1 finding now REJECTS: one regeneration with the quoted
        // finding fed back (`v2Directive`), and a second J1 serves the floor - the
        // same single v2 regeneration every D1-D4 rejection draws on. J2-J4 stay
        // ADVISORY exactly as since 1.29.0: stored at `flag`, never blocking.
        //
        // A JUDGE THAT CANNOT RUN NOW GATES TOO: with J1 a gate, "the judge was
        // down" means J1 was never checked, and judge.js's own contract is that the
        // caller treats that as "not validated", never as a pass (rule 17). J2-J4
        // being advisory is why this was harmless before and is not now.
        const gating = (f) => f.class === 'J1' || f.check === 'v2.judge_unavailable';
        judged.findings = judged.findings.map((f) => ({
          ...f, judge_severity: f.severity, severity: gating(f) ? 'hard' : 'flag',
        }));
        gate.findings = [...gate.findings, ...judged.findings];
        if (judged.findings.some(gating)) {
          gate.ok = false;
          gate.hard = true;
        }
      }
      if (gate.ok) {
        attempts.push({
          ...
          ...(judged ? { judge_usage: judged.usage, judge: judged.findings, judge_malformed: judged.malformed } : {}),
          ...wroteAnyway,
        ...
        return {
          ...
          prompt_version: promptVersionFor(semanticJson.kind, semanticJson.voice),
          stage6_version: gate.stage6_version,
          findings: gate.findings, // 'flag' findings can survive a pass
          // VOICE v2: the review is STORED WITH THE RENDER (spec §4b), so any
          // acceptance or rejection can be read and argued with later.
          ...(isV2 ? {
            review: {
              deterministic: gate.findings.filter((f) => !String(f.check).startsWith('v2.judge_')),
              judge: judged?.findings ?? [],
              judge_malformed: judged?.malformed ?? [],
              rejected_drafts: rejectedReviews,
            },
          } : {}),
          cache_key: key,
          cached: false,
      ...
      if (isV2) rejectedReviews.push({ findings: rejecting });
      if (regenerationsLeft > 0) {
        ...
        directive = isV2
          ? v2Directive(gate.findings)
          : stricterDirective(gate.findings, forbiddenLiterals(semanticJson));
        continue;
      }
  ...
  return {
    // THE SAME OPENING ON THIS PATH TOO. The floor already assembles `p0_opening`
    // from the same cell, so this is byte-identical today (asserted, not assumed:
    // tests/compat-p0-engine.spec.mjs). It runs anyway because that identity is a
    // property of `required_points` ordering, and the guarantee being made is
    // "every pair reading out of this module opens with the ruled sentence" - a
    // guarantee that holds only if it does not depend on which path was taken.
    ...withEngineOpening(assembleFallback(semanticJson), semanticJson).rendered,
    model: null,
    prompt_version: null,
...
export async function persistRendered(rendered, semanticJson) {
  ...
  await writeCache(rendered.cache_key, {
    ...
    // VOICE v2 only: the reviewer's findings, stored with the render. A v1 row
    // carries none, so v1 writes are byte-for-byte what they were.
    ...(rendered.review ? { review: rendered.review } : {}),
  });
  return true;
}
