...
async function renderOnce(semanticJson, {
  ...
  // Findings of REJECTED v2 drafts, kept so a regeneration can be argued with.
  ...
  for (const provider of chain) {
    ...
    for (;;) {
      ...
      // ── NO JUDGE IN THE RENDER PATH (STAGE6 1.42.0, Reyner 2026-09-26, MVP) ──
      // docs/product/product-boundary-rulings-2026-09-26.md item 5: engine ->
      // Flash-lite writer -> deterministic checks -> serve. The semantic judge that
      // ran here from 1.29.0 (advisory) and gated on J1 from 1.33.0 is gone from
      // every path; lib/validate/judge.js and scripts/calibrate-j1.mjs stay as QA
      // tools run by hand on samples. The engine-provable errors J1 used to catch
      // on pairs are deterministic checks now (lib/validate/pairTruth.js). A
      // production judge returns only on post-launch evidence that a problem
      // affects customers or revenue. tests/voice-judge.spec.mjs counts the calls.
      if (gate.ok) {
        attempts.push({
