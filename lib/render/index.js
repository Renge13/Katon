...
async function renderOnce(semanticJson, {
  ...
  for (const provider of chain) {
    ...
    for (;;) {
      ...
      // ── VOICE v2: THE JUDGE, J1-J4 (spec §4b) ─────────────────
      // Only once D1-D4 pass: a draft already rejected on a hard deterministic
      // check is not worth a second model call. A judge that cannot run means the
      // draft is NOT VALIDATED (rule 17) - it goes to the floor, and no
      // regeneration is spent on a reviewer that is down.
      let judged = null;
      if (isV2 && gate.ok) {
        try {
        ...
        const failingJudge = judged.findings.filter((f) => f.severity !== 'flag');
        gate.findings = [...gate.findings, ...judged.findings];
        if (failingJudge.length > 0) {
          gate.ok = false;
          gate.hard = gate.hard || failingJudge.some((f) => f.severity === 'hard');
        }
      }
      if (gate.ok) {
        attempts.push({
