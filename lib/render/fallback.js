...
//
// ── RULE 21, AND THE AMBIGUITY THAT COST TWO IMPLEMENTATIONS ──
// "lemah"/"kuat" may never render bare; the explanation lands in the same breath
// (glossary `kekuatan._note`). That note requires the label to co-occur with its
// meaning **"in rendered text"**, and it did not say whether a HEADING is rendered
// text. Both readings are defensible and each produced a different floor inside
// one day: first the label was repeated in the body because a heading was assumed
// not to count, then the heading was deleted because the body already named it.
//
// **RULED BY REYNER 2026-08-26: A HEADING DIRECTLY ABOVE A MEANING PARAGRAPH
// FULLY SATISFIES "SAME BREATH."** Visually and cognitively they are coupled. The
// ruling is in `docs/PROGRESS.md` and beside the note itself, so it is written
// down rather than inferred a third time.
//
// What follows from it, and it is the whole of this file's naming policy:
//   - THE HEADING NAMES THE FACT. `assembleFallback` sets it to `fact.label`.
//     Headings stay: they are the document's structure and the reason a
//     five-section reading is scannable on a phone.
//   - THE BODY DOES NOT REPEAT IT. The bare label sentence that used to precede
//     `label_meaning` is deleted for every cell.
//   - A LABEL-LESS FACT IS A CONDITION and is described, never named, exactly as
//     before. Those blocks have always shipped heading-less.
// ============================================================
 ...
 */
// EXPORTED for lib/render/pairOpening.js and nothing else. The compat opening is
// composed by this function rather than by a second formatter, so the floor's own
// first block cannot drift away from the one the model path is handed.
export function blockFor(fact) {
  const parts = [];
  ...
  // ── THE BARE LABEL SENTENCE IS GONE, FOR EVERY CELL ──
  //
  // RULED BY REYNER 2026-08-26: **a heading directly above a meaning paragraph
  // fully satisfies rule 21's "same breath"** - visually and cognitively they are
  // coupled. `kekuatan._note` required the label to co-occur with its meaning "in
  // rendered text" and never said whether a heading IS rendered text. It is. That
  // ambiguity drove two different implementations of this same function inside one
  // day before it was written down; the ruling now lives in `docs/PROGRESS.md` and
  // beside the note itself.
  //
  // So the block's HEADING names the fact (see the push in `assembleFallback`) and
  // this sentence only repeated it, one line below:
  //
  //     ASPEK PENGATUR
  //     Aspek Pengatur (Direct Officer). Kamu tahu apa yang seharusnya ...
  //
  // UNCONDITIONAL, AND THAT SUPERSEDES THE 2026-08-26 CONDITIONAL SUPPRESSION.
  // That condition - suppress only where `label_meaning` already opens with the
  // label - existed solely because the heading was assumed not to count. It does
  // count, so the condition goes with it, and `meaningOpensWithLabel` goes with
  // the condition.
  //
  // WHAT IT HOLDS, all three checked by tests rather than argued:
  //   - every badge is still NAMED, in its heading. A badge is something the
  //     person HAS and naming it is deliberate.
  //   - rule 21 holds for `kekuatan.strong` and `balanced`, whose meanings never
  //     restated the label - the heading carries it now.
  //   - the archetype identity clause is untouched. It is a ruled SENTENCE, not a
  //     bare label, and it is what stops the floor opening on a noun phrase.
