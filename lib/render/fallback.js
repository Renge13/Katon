...
// glossary entry - `RENDER_COPY.floorIdentity`, RULED BY REYNER 2026-08-21. They are
// there because the floor's opening was two bare noun-phrases with no verb
// ("Matahari (The Sun)." "Api (Fire).") and he ruled that shape unsellable, and
...
//
// ── THE BOUNDARY, RULED 2026-08-21. THIS IS THE LIMIT, NOT AN OPENING. ──
// The floor MAY draw CONNECTIVE TISSUE from `RENDER_COPY`: punctuation, copulas,
// prepositions. The words that let two engine strings stand in one sentence.
//
// It may NEVER draw A CLAIM. Nothing that asserts anything about the reader - no
// trait, no verdict, no consequence, no timing, no advice, and nothing that would be
// true of one person and false of another. Those are glossary content, ruled per cell
// against a chart, and a sentence assembled here has no chart behind it.
//
// The test is mechanical: strip every engine string out of the assembled sentence and
// read what is left. If the remainder says something about her, it does not belong in
// this file. "Kamu adalah ... dengan unsur ..." leaves a copula and a preposition and
// asserts nothing on its own. "Kamu cenderung ..." would assert a disposition, and
// belongs in a glossary cell Reyner ruled or nowhere.
//
// So the contract is not "authors nothing" with an exception bolted on. It is: this
// file may supply GRAMMAR and never MEANING.
//
...
// (glossary kekuatan._note). The assembly keeps the label and `label_meaning`
// adjacent inside ONE text field, so the resolution cannot be separated from the
// label by a layout decision. The label is therefore spoken in the TEXT and not
// left to the heading: a heading is a separate visual element, and a reading that
// relies on one for its resolution has a bare label.
//
// AMENDED 2026-08-26. That used to mean "the label sentence is always emitted",
// and it produced "Lemah (Weak). Lemah di sini bukan berarti tidak mampu." -
// the word three times in eleven characters, which Reyner read on production and
// ruled out. The requirement was never the SENTENCE, it was the label being
// present in the text beside its meaning. Where `label_meaning` already opens by
// naming itself the sentence is pure duplication and is suppressed; where it does
// not, the sentence is the only place the term is spoken and it stays. See
// `meaningOpensWithLabel` and the note above the emit. Two of thirty-six cells
...
/**
 * Does this fact's `label_meaning` already open by naming the fact?
 *
 * If it does, the bare label sentence in front of it is a duplicate and is
 * suppressed (ruled 2026-08-26). If it does not, that sentence is the only place
 * the term is spoken in the prose and it stays.
 *
 * TWO OF THIRTY-SIX cells restate their label: `kekuatan.weak` and
 * `relasi_cabang.害`. The comparison is a prefix match on the label rather than
 * anything cleverer, because the two real cases are literally "Lemah ..." and
 * "Gesekan ...", and a looser test (the label appearing anywhere in the meaning)
 * would suppress introductions that merely mention the word later on.
 */
function meaningOpensWithLabel(fact) {
  if (!fact.label || !fact.label_meaning) return false;
  return fact.label_meaning.trimStart().toLowerCase()
 ...
 *
 * @returns {{ text: string, namesFact: boolean }} `namesFact` is true when the
 *   block itself says what it is about, which is what lets the caller drop a
 *   heading that would repeat it.
 */
function blockFor(fact) {
  const parts = [];
  // Set wherever this block SAYS what it is about. The caller drops the heading
  // when it is true, because the heading would then be the second time in two
  // lines - see the note on the block push.
  let namesFact = false;
  ...
  // and were RULED BY REYNER on 2026-08-21. This file still contributes only
  ...
  if (fact.label && !fact.archetype?.name_id && !meaningOpensWithLabel(fact)) {
    parts.push(fact.label_bracket
      ...
      : sentence(fact.label));
    namesFact = true;
  } else if (fact.label && meaningOpensWithLabel(fact)) {
    // Suppressed above precisely BECAUSE the meaning opens by naming the fact,
    // so the block still names it - just in the meaning rather than in front of it.
    namesFact = true;
  }
  ...
  const text = needsBreak
    ? `${parts.slice(0, giftAt).join(' ')}\n\n${parts.slice(giftAt).join(' ')}`
    : flat;

  // `namesFact` is reported STRUCTURALLY rather than by searching the finished
  // text for the label. The three ways this block can name its fact are all
  // decided above, in code, and a string search would additionally match a label
  // that merely appears somewhere in a glossary sentence - which is a mention,
  // not a naming, and is not grounds for dropping the heading.
  return { text, namesFact };
}
...
export function assembleFallback(semanticJson) {
  ...
  for (const point of semanticJson.required_points || []) {
    ...
    const { text, namesFact } = blockFor(fact);
    if (text === '') continue;
    ...
    // ── NO HEADING WHERE THE BLOCK ALREADY NAMES ITSELF ──
    // RULED 2026-08-26. The heading was `fact.label` unconditionally, and the
    // body names the same fact one line below it:
    //
    //     ASPEK PENGATUR
    //     Aspek Pengatur (Direct Officer). Kamu tahu apa yang seharusnya ...
    //
    // That is the stutter Reyner reported. The NAMING SENTENCE STAYS - it is what
    // rule 21 and the badge-naming contract depend on - and the duplicate heading
    // goes.
    //
    // MEASURED: 33 of the 37 distinct floor blocks across the fixture carry a
    // heading, and the body names the fact in ALL 33. There is no case where the
    // heading is the only mention, so in practice this removes every heading the
    // floor emits. That is the honest consequence rather than a partial one, and
    // it is why the count is in the commit message as well as here.
    //
    // A HEADING-LESS BLOCK IS ALREADY A FIRST-CLASS CASE. The four label-less
    // condition blocks have always shipped without one, `mirrorServeView` maps
    // `heading: block.heading || ''`, the funnel passes `eyebrow={b.heading ||
    // undefined}`, and `scripts/qa-renders.mjs` omits the marker rather than
    // printing it empty - a 2026-08-21 fix for exactly this.
    blocks.push({ fact_ids: [fact.id], heading: namesFact ? '' : (fact.label || ''), text });
  }
