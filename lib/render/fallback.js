...
function blockFor(fact) {
  ...
  //
  // AUTHORS NOTHING, same contract as the palace line above. `name_id` and
  // `name_en` are glossary strings and this contributes the ordering and a full
  // stop. The element still arrives, in the very next sentence, which is the
  // "next breath" the ruling asks for rather than in front of the image.
  //
  // NOT the element's own bracket. `Elemen` stays bracketed here for now because
  // rule 23's ruled scope - Aspek, Bintang, Arketipe, and explicitly NOT Pilar or
  // Elemen - is enforced in its own commit with its own measurement. Changing it
  // here would ride a second change on this one.
  if (fact.archetype?.name_id) {
    parts.push(fact.archetype.name_en
      ? sentence(`${fact.archetype.name_id} (${fact.archetype.name_en})`)
      : sentence(fact.archetype.name_id));
  }
