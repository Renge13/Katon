...
export async function renderWithGemini(systemPrompt, semanticJson, config) {
  ...
  // `usage`: Gemini's own token counts, so a measurement can report spend without
  // estimating it. Additive; nothing on the serve path reads it.
  return { text, provider: 'gemini', model: config.model, usage: payload?.usageMetadata ?? null };
}
