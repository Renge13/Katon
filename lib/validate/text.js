 ...
 *
 * Added 2026-08-21 for the rule-23 bracket check, and the reason is worth keeping:
 * a heading is a separate visual element, not a sentence. The module-assembly floor
 * sets `heading: fact.label` and then REPEATS the label inside the text, bracketed,
 * exactly because (fallback.js) "a reading that relies on one for its resolution has
 * a bare label". Measured over the whole reading INCLUDING headings, the first
 * mention of "Aspek Pengatur" is the bare heading and the correctly bracketed
 * sentence one line below never gets looked at - so bracket-once would reject the
 * always-available floor on every chart, which rule 17 does not allow.
 *
