 ...
 */
export const STAGE6_VERSION = '1.0.0';
...
export function validateRendering(rendered, semanticJson, { provider = 'gemini' } = {}) {
  ...
  const { findings: structural, normalized } = structureGuard(rendered);
  const text = renderedText(normalized);

  const findings = [
    ...factGuard(normalized, semanticJson, text),
    ...forbiddenGuard(text),
    ...coverageGuard(normalized, semanticJson),
