...
import { bracketGuard } from './brackets.js';
 ...
 * of it. It was a reporter, and its enforcement is 1.12.0 below.
 *
 * ── 1.12.0: RULE 23 BRACKET-ONCE IS ENFORCED ───────────────
 * 2026-08-21. `brackets.unbracketed` and `brackets.mismatch` go from `flag` to
 * `soft`, so readings that used to pass now fail. Reyner ruled it on the sentences
 * rather than on the principle - `Kamu adalah Matahari (The Sun) yang Lemah` over
 * `Kamu adalah Api Matahari yang Lemah` - and the defect it catches is the FUSED
 * element+archetype opening rather than a forgotten bracket. Reporting first is what
 * produced the number the ruling was made on, and it corrected the diagnosis on the
 * way; the history is in lib/validate/brackets.js.
 *
 * `no_pair` STAYS NON-FAILING. Rejecting a term the payload supplies no English for
 * would oblige the renderer to invent terminology, which is rule 14 inverted.
 *
 * ONE ACCEPT-CHANGING EDIT, which is the whole commit. The CLAUDE.md rule-23 scope
 * amendment rides with it because it is the documentation OF this change and cannot
 * move a floor rate - not because two changes were bundled.
 */
export const STAGE6_VERSION = '1.12.0';
...
export function validateRendering(rendered, semanticJson, { provider = 'gemini' } = {}) {
  ...
  const metrics = {
    ...
    // whether it passed or failed, because the threshold-fitting harness needs both -
    // and since 1.12.0 unbracketed and mismatch also REJECT. See brackets.js.
    brackets: [],
  ...
  const findings = [
    ...
    // not an unbracketed first mention, and treating it as one rejects the floor.
    ...bracketGuard(semanticJson, renderedProse(normalized), metrics),
