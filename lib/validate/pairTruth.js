...
/** For other guards: `clauses`, and the one side a text names ('a' | 'b'), or null if none or both. */
export { clauses };
export function namedSide(text) {
  const sides = new Set(sideMarks(text).map((m) => m.side));
