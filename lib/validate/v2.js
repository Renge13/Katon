...
function normaliseSquareGlosses(rendered, semanticJson) {
  ...
  const fix = (text) => {
    ...
    return text.replace(/[ \t]*\[([^\]\n]{1,60})\]/gu, (m, inner, offset, whole) => {
      ...
      const to = term ? ` (${glosses.get(term)})` : '';
      changes.push({ term: term ?? null, was: m.trim(), to: to.trim() || null });
      return to;
    });
  };
  return {
...
export function validateRenderingV2(rendered, semanticJson, { provider = 'gemini' } = {}) {
  ...
  const { rendered: normalized, changes: typo } = normaliseTypography(squared);
  ...
  const bracketLog = [
    ...
    ...square.map((s) => finding('brackets.square_normalised', 'flag',
      s.to ? `replaced "${s.was}" after "${s.term}" with "${s.to}"` : `removed "${s.was}" (no sanctioned bracket)`,
      s.term ? [s.term] : null)),
  ...
  const findings = [...inventedTerms(text, suppliedTerms(semanticJson)), ...d2, ...d3, ...d4, ...facts, ...logged];
