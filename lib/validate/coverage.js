 ...
 */
export function coverageGuard(rendered, semantic) {
  const out = [];
...
function slotFillingFlag(rendered, semantic) {
  const factOrder = (semantic.facts || []).map((f) => f.id);
  const leadIds = (rendered.blocks || []).map((b) => (b.fact_ids || [])[0]).filter(Boolean);
