...
import { palaceName } from '@/lib/semantic/glossary';
...
// term_side must be replayed here or a revisit would recompute a DIFFERENT month
// pillar than the one the user resolved at the season gate.
function chartFromRow(row) {
  return calculateBaziChart({
    birthDate: row.birth_date,
    birthTime: row.birth_time || null,
    termSide: row.term_side || null,
    // Replayed for completeness. It does not affect the natal chart, so unlike
    // term_side it cannot change what a revisit renders.
...
export function freeChartView(chart, stem = chart.day.stem) {
  ...
  return {
    ...
    palaces: buildPalaces(chart),
  };
}

/**
 * 胎元 for the chart sheet. DISPLAY ONLY — it appears in Joey's "Personal Chart
 * Details" block and a cross-checking user notices its absence, which is what
 * that block is for. No interpretation is attached anywhere downstream.
 *
 * 命宮 was removed (D1b): no candidate convention reproduces Joey better than
 * 4/5, and in a legitimacy block a wrong value is worse than an absent one.
 * Do not re-add it without settling the convention against 10+ Joey values.
 */
export function buildPalaces(chart) {
  const p = (label, palace) => (palace
    ? { label, stem: palace.stem, branch: palace.branch, element: palace.element,
        elementId: ELEMENT_ID[palace.element], animal: palace.animal }
    : null);
  return {
    // Indonesian label; the hanzi stays in the cell as the cross-checkable
    // object, paired with its element and animal (CLAUDE.md rule 23).
    //
    // The label comes from the glossary as of 2026-08-07. It used to be the
    // literal 'Istana Konsepsi', hand-authored here and present in no glossary
    // entry - which is why the mirror route declined to reuse it. Reyner ruled
    // the string: it is 'Pilar Konsepsi', and it now has one home.
    konsepsi: p(palaceName('conception'), chart.conceptionPalace),
  };
}
