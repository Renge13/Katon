...
/**
 * 命宮 / 胎元 for the chart sheet. DISPLAY ONLY — both appear in Joey's "Personal
 * Chart Details" block and a cross-checking user notices their absence, which is
 * what that block is for. No interpretation is attached anywhere downstream.
 *
 * lifePalace is null when the birth hour is unknown; the client must render that
 * as an empty slot rather than hiding the row, the same way the hour pillar does.
 */
export function buildPalaces(chart) {
  const p = (label, palace) => (palace
    ...
    : null);
  return {
    // Indonesian labels; the hanzi stays in the cell as the cross-checkable
    // object, paired with its element and animal (CLAUDE.md rule 23).
    hidup: p('Istana Hidup', chart.lifePalace),
    konsepsi: p('Istana Konsepsi', chart.conceptionPalace),
    hasLifePalace: chart.lifePalace !== null,
  };
}
