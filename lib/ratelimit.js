...
export const RATE_LIMITS = {
  ...
  // ── CHECK-STATUS ON LOAD (launch-cut item 4, 2026-09-23) ──
  // Each call to POST /api/pair/<id>/reconcile can be a DOKU check-status request,
  // so a valid pair id in a loop is a request to DOKU in a loop. The page asks ONCE
  // per load; thirty an hour per IP is a reader reloading a slow payment page many
  // times over and nothing like a loop. IP only, for deliver_pdf's reason (one buyer,
  // two devices). UNFITTED, like every number in this table.
  pair_reconcile: {
    ip: { limit: 30, windowSeconds: 3600 },
  },
  // The Rp 19.000 purchase's twin (PAY-SAFETY-ALL-PURCHASES, 2026-09-23). Its OWN
  // bucket, same numbers: a reader with a pair AND a mirror purchase open must not
  // spend one product's allowance on the other.
  reading_reconcile: {
    ip: { limit: 30, windowSeconds: 3600 },
  },
