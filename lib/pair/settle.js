...
//    genuine provider double-fire cannot run a once-only side effect twice.
//
...
/**
 * Settle a compat purchase against the provider's own record.
 *
 * @param {string} pairId the pair id (the invoice's external_id)
 * @param {Object} pairRow the pair row, already loaded
 * @param {boolean} statusPaid the provider's own record says the money arrived
...
export async function settlePair(pairId, pairRow, statusPaid, settledAmount) {
  ...
  if (statusPaid && !amountOk) {
    ...
    console.error(
      `[settle] amount rejected for pair ${pairId}: sku=${pairRow.sku ?? 'null'}`,
    );
  ...
  // flipped. A notification that 500s over a failed render would have the provider retry a
