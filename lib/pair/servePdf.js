 ...
 */
export async function servePairPdf(request, id, { renderPdf }) {
  ...
  // After a bump this refuses until the reading is re-rendered, which is the honest
  ...
  const rendered = await readCache(cacheKey(semanticJson), { includeUnvalidated: false });
