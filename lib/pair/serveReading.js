import 'server-only';
import { projectNames } from './reportView.js';
// ============================================================
...
export async function servePairReading(request, id, { renderOptions = {} } = {}) {
  ...
  return reply({
    ...
    //
    // ── IT SHIPPED EMPTY IN #113, AND IT IS A CALLABLE NOW BECAUSE OF THAT ──
    // The expression here read `f.entry?.name_id`. `fact()` spreads
    // `contentFrom(entry)` and keeps no `entry` key, so it was undefined for
    // every fact and this field was `{}` on every pair - P1/P2/P3 rendered an
    // eyebrow with no headline, and P4/P5 looked right only because the
    // component falls back to `facts.pattern`/`facts.quadrant`. The fallback
    // masked it on the two blocks anyone would check first.
    //
    // `projectNames` is in `lib/pair/reportView.js` so a test can CALL it
    // against a real `buildPairSemantic` output. An inline expression here could
    // only be checked by a fixture, and the fixture that reviewed this one was
    // hand-written to the shape it expected.
    names: projectNames(semanticJson),
    reading: {
