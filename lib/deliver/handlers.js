...
// It does not touch `INVOICE_DESCRIPTION`. That string is on the provider's checkout
 ...
 */
/** The reading page's own door: renders, gates and persists as a page load does. */
const readingDoor = async (request, id) => {
  const { serveMirrorReading } = await import('../mirror/handlers.js');
  return serveMirrorReading(request, id);
};

export async function serveDeliveryPdf(request, id, { renderPdf, warm = readingDoor }) {
  const row = await getReading(id);
  if (!row) return notFound();
  ...
  // would otherwise come from two different engine versions in one document.
  // ── ~~After a bump this refuses until the reading is re-rendered~~ ─────
  // CORRECTED 2026-09-24: a miss is WARMED THROUGH THE READING DOOR, once. Reyner's
  // own paid pair answered 409 on production - her reading was cached under an
  // older ENGINE_VERSION, and only the READING page renders, so a pre-bump buyer who
  // tapped "Unduh PDF" first got JSON. The PDF still renders NO PROSE OF ITS OWN
  // and still never mixes engine versions: it asks the READING page's own door
  // (`serveMirrorReading`), which renders, runs Stage 6 and persists exactly as a page load
  ...
  // THE ROW'S GENDER, for the cover's birth line (round-1 markup R3, 2026-09-24):
  // the chart does not carry it, and `birthSummary` prints it when given.
  const { buffer } = await renderPdf({
    chart, semanticJson, rendered, gender: row.gender || null,
  });
