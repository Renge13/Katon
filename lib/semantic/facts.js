...
import {
  GLOSSARY, ELEMENT_HANZI, contentFrom, elementId, palaceName,
} from './glossary.js';
...
export function buildFactInventory(chart, strength = computeStrength(chart)) {
  ...
  // NO GLOSSARY ENTRY EXISTS. glossary.json has no `kekuatan` section, so there
  // is no label, no label_meaning and no gift/cost for weak / balanced / strong.
  // The target file supplies them by hand ("Api Lemah" / "Weak Day Master" plus
  // three bespoke paragraphs) and CR-5 makes this the single most delicate string
  // in the product — "lemah" is permitted only when the explanation lands in the
  // same breath. It is flagged, not invented.
  facts.push(fact({
  ...
  for (const punishment of chart.punishments ?? []) {
    ...
    facts.push(fact({
      ...
      provenance: {
        ...
        palaces: [...new Set(positionsOf(chart)
          .filter(([, p]) => punishment.branches.includes(p.branch))
          .map(([pos]) => pos))].map(palaceName),
      },
  ...
  if (decisiveVerdict && strength.unfavorable.includes(profile.element)) {
    facts.push(fact({
      ...
      extra: { coherence_rule: 'CR-1', supersedes: 'main_profile' },
    }));
