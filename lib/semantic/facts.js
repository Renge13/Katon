 ...
 * Until now the hierarchy's actionability axis read `fact.actionable`, which is
 * the PROSE from `glossary.actionable_seed`. So a fact counted as actionable if
 * and only if someone had written its actionable sentence yet, and authoring one
 * re-ranked every chart carrying that cell. Measured on the tranche-1 content
 * pass: fact order moved on 11 of 13 charts, for no reason that had anything to
 * do with the charts.
 *
 * Rule 14 says the engine owns order. Reyner's ruling, 2026-08-11: order must be
 * a function of the CHART, not of which prose has been authored yet. So
 * actionability is declared here, beside the firing gates, and the hierarchy
 * reads this table instead of the prose.
 *
 * Keyed on `provenance.kind`, which is the domain the fact came from. That is
 * deliberate and it is why the table cannot reproduce today's values: 15 badge
 * facts carry prose and 12 do not, because 桃花, 天乙貴人 and 空亡 have seeds and
 * the other badges do not. Under a declaration all badges answer the same way,
 * which is the whole point - "can a reader act on a badge" is a question about
 * badges, not about which cells have been written.
 *
 * NOTHING IS DECLARED TRUE YET. This commit installs the mechanism and leaves
 * every value false, so the axis contributes nothing until the values are ruled.
 * That is a real ranking change and it is measured as one - see the PR. The
 * values are a separate commit because they are a separate decision (rule 13).
 */
export const ACTIONABLE_KINDS = {
  aspek_convergence: false,
  badge_anchor: false,
  branch_relation: false,
  coherence_rule: false,
  day_branch_seat: false,
  day_stem: false,
  element_absent: false,
  element_dominant: false,
  month_branch_rooting: false,
  punishment: false,
  strength: false,
  void_stack: false,
};
...
function fact({ id, type, provenance, entry, palace = null, palaces = null, extra = {} }) {
  ...
  // ── 5. A dominant element. Gated, see FACT_GATES. ──
  {
    ...
    if (pct >= FACT_GATES.elementDominantPct) {
      ...
      facts.push(fact({
        ...
        // Reuses the element's own character entry. There is no separate
        // "you have a lot of this" entry in the glossary; the element's
        // label_meaning is the closest true thing and it is not wrong, but it
        // does not say "and you have a great deal of it".
        entry: GLOSSARY.elemen[hanzi],
        extra: { glossary_source: 'elemen', internal_only: ['provenance.percent'] },
      }));
    }
  }
