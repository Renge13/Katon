...
function identityFirst(facts, core, verdict) {
  const identity = identityFacts(facts, core, verdict);
  return [...identity, ...facts.filter((fact) => !identity.includes(fact))];
}

/**
 * The identity spine itself: at most one fact per IDENTITY_MATCHERS limb, in spine
 * order. Split out of `identityFirst` (2026-09-26) so the v2 required set reads the
 * SAME definition of "the three opening facts" instead of a second one.
 */
function identityFacts(facts, core, verdict) {
  const identity = [];
 ...
 */
export function buildSemanticJson(chart, options = {}) {
  const { voice = voiceVersion(), ...rest } = options;
  return markVoice(buildSemanticJsonV1(chart, rest), voice);
}

/**
 * ── THE VOICE MARKER (voice v2 round 2, 2026-09-24) ─────────
 * v1 returns the object UNTOUCHED, so a v1 payload - and therefore every v1 cache
 * key - is byte-identical to what `main` produced (pinned in
 * tests/voice-cache.spec.mjs). v2 gains `voice: 'v2'`, which `cacheKey` hashes, so
 * the two voices can never share a row in either direction.
 */
export function markVoice(semanticJson, voice) {
  if (voice !== 'v2') return semanticJson;
  return {
    ...semanticJson,
    voice: 'v2',
    // ── E1 (spec §5): `actionable` leaves must_cover ─────────
    // Actionables stay on the facts as material the writer MAY use; they are no
    // longer something a reading must contain (worksheet §2.3: 8 of 9 required
    // points demanded homework).
    required_points: (semanticJson.required_points || []).map((p) => ({
      ...p,
      must_cover: p.must_cover.filter((m) => m !== 'actionable'),
    })),
  };
}

/**
 * The pair facts a v2 reading must cite. Cowork, AD amendment 1 (2026-09-26):
 * `p0_opening` is already enforced by `pair.both_named` and `p2_reframe` by
 * `pair.reframe_missing`, both hard, so neither also needs a D2 citation.
 */
const V2_PAIR_CITED = ['p2_day_pair', 'p5_pull_fit'];

/**
 * The required points v2's D2 checks for a citation (Prompt AD amendment 1,
 * item 2, 2026-09-26). Mirror: the three identity facts, by the same matchers that
 * put them first in facts[] - NOT everything tagged `role 'spine'`, which also
 * carries `spouse_palace` and `profile_vs_favorable`. Pair: V2_PAIR_CITED.
 *
 * ── A FILTER FOR D2, NOT A SMALLER `required_points` ──────
 * `required_points` itself is unchanged, on purpose: lib/render/fallback.js builds
 * the FLOOR from it (one block per point), so shrinking the list would cut the v2
 * floor - the reading every reader gets during an outage - to two or three blocks.
 * The other points stay material the writer may use and the floor still covers.
 *
 * @param {Object} semanticJson a mirror or pair semantic JSON
 * @returns {Set<string>} fact ids
 */
export function v2CitedPointIds(semanticJson) {
  if (semanticJson.kind === 'pair') return new Set(V2_PAIR_CITED);
  return new Set(identityFacts(semanticJson.facts || [], semanticJson.core || {}, semanticJson.strength?.verdict)
    .map((f) => f.id));
}

function buildSemanticJsonV1(chart, {
...
export function cacheKey(semanticJson) {
  ...
  // THE VOICE IS HASHED IN EXPLICITLY for v2 (voice v2 round 2, 2026-09-24). It is
  // in `canonical` too, but the key's isolation must not depend on a field surviving
  // canonicalisation. v1 carries no `voice`, so its prefix - and its key - is unchanged.
  const voice = semanticJson.voice ? `voice=${semanticJson.voice}\n` : '';
  return createHash('sha256')
    .update(`${voice}${semanticJson.engine_version}\n${canonical}`)
    .digest('hex');
}
