...
import { branchesToTaggable } from '@/lib/zodiac';
...
export function getFreeContent(stem, elementVariant, domain, opts = {}) {
  ...
  const compat = harmonyBranches || a.card?.compatibleBranches;
  const clash = clashBranches || a.card?.clashBranches;
  const card = {
    ...a.card,
    compatibleBranches: compat,
    clashBranches: clash,
    // archetype-primary taggable lists (archetype name + zodiac hint) for the card
    selaras: branchesToTaggable(compat),
    pemicu: branchesToTaggable(clash),
  };
  return {
