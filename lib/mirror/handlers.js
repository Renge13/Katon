...
export async function createMirrorReading(request) {
  if (!previewAllowed(request)) return gone();
  ...
  if (birthTime !== null && (typeof birthTime !== 'string' || !TIME_RE.test(birthTime))) {
    return badRequest('birthTime must be HH:MM (24h) or omitted');
  }
  ...
  try {
    ...
    return badRequest(`chart could not be computed: ${err.message}`);
  }
  ...
  return reply({ token: id, path: `/api/mirror/${id}` }, 201);
}
...
export async function serveMirrorReading(request, token) {
  ...
  try {
    ...
    if (err instanceof RenderRefused) return reply({ error: err.reason }, 503);
    throw err;
  }
  ...
  return reply(mirrorServeView({ token, chart, semanticJson, rendered }));
}
