...
import { validateRenderingV2 } from '../validate/v2.js';
 ...
 */
/**
 * The gate of the voice that WROTE the text (STAGE6 1.43.0, Prompt AG item 4).
 *
 * Until 1.43.0 both serve-time re-checks below ran `validateRendering` whatever
 * the voice, so a cached v2 reading was judged by v1's rules: a v2 row that passed
 * v2's gate when written could be floored on a later serve by a check v2 only
 * logs (tests/voice-cache.spec.mjs). The row's voice is the semantic JSON's: the
 * voice is part of the cache key, so a row found under this key was written by it.
 */
function gateFor(rendered, semanticJson, provider) {
  const validate = semanticJson?.voice === 'v2' ? validateRenderingV2 : validateRendering;
  return validate({ blocks: rendered.blocks, penutup: rendered.penutup }, semanticJson, { provider });
}

export function floorIfHardFailing(rendered, semanticJson) {
  if (!rendered.cached || rendered.source === 'module_assembly') return rendered;

  const gate = gateFor(rendered, semanticJson, rendered.source);
...
export function floorRefusalReason(rendered, semanticJson) {
  ...
  const gate = gateFor(rendered, semanticJson, 'module_assembly');
