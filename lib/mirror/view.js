...
//
// 胎元 IS here as of 2026-08-07. It was left out of the first pass because its
// only Indonesian label was hand-authored in lib/readingView.js and existed in
// no glossary entry, and register is Reyner's (rule 20). He ruled it: "Pilar
// Konsepsi", now in GLOSSARY.pilar.conception, read from there by both surfaces.
// It sits OUTSIDE `pillars[]` because it is not one of the four chart positions
// - it is derived from the month pillar and is display-only.
// ============================================================

import { splitParagraphs } from '../render/paragraphs.js';
import { GLOSSARY, elementId, palaceName } from '../semantic/glossary.js';
...
export function mirrorChartView(chart, semanticJson) {
  ...
  return {
    ...
    // 胎元. Display only, and 5/5 against Joey's printed values - it is in the
    // legitimacy block because a cross-checking reader notices its absence, and
    // nothing downstream interprets it. 命宮 is deliberately NOT here and must
    // not be added: two candidate conventions score 4/5 and 3/5 against Joey's
    // own values, and in a block whose only job is to be checkable a wrong value
    // is worse than an absent one (CLAUDE.md rule 4, prompts/D1b).
    conception_pillar: conceptionCell(chart.conceptionPalace),
...
/** The 胎元 cell, paired the same way a pillar cell is (rule 23). */
function conceptionCell(palace) {
  if (!palace) return null;
  return {
    label: palaceName('conception'),
    hanzi: `${palace.stem}${palace.branch}`,
    stem: palace.stem,
    branch: palace.branch,
    // Read from the glossary rather than from the chart's own `animal` field,
    // which comes from lib/zodiac.js. The two agree today (checked 2026-08-07,
    // all twelve branches); the glossary is the one the rest of this file uses
    // and a second source is how a drift starts.
    animal: GLOSSARY.shio[palace.branch]?.name_id ?? null,
    element: elementId(palace.element),
  };
}

/**
 * Card A's data: everything `buildCardData` produces except the paid appendix and
 * the birth date. See the note at the `card` field for why each is withheld.
 */
function freeCardData(chart, semanticJson) {
  const full = buildCardData({
    chart, semanticJson, birthDate: null, gender: null,
  });
  // Deleted rather than destructured-and-dropped, so the withholding is one
  // statement a reader can grep for instead of an unused binding a linter argues
  // about. `appendix` is Card B's half; Card A never reads it.
  delete full.appendix;
  return full;
}
...
export function mirrorServeView({ token, chart, semanticJson, rendered }) {
  ...
  return {
    token,
    // ── THE FREE SHAREABLE, AS DATA ────────────────────────
    // Card A. It replaces `components/Sharecard.jsx`, which read the retired
    // `contents/*.md` cells, and it is the reason the card and the reading can no
    // longer disagree about her archetype's name: both now resolve it from
    // `glossary.json` through Stage 3.
    //
    // TWO THINGS ARE WITHHELD, and each for its own reason.
    //
    //   `appendix`   Card B's half of the payload - the four pillar characters
    //                with their palaces and the element distribution. It is the
    //                paid card's substance, so it is served by
    //                /api/deliver/[id]/card behind `row.paid === true` and not
    //                here. Card A never reads it.
    //   the FOOTER   built with `birthDate: null`, so no birth date leaves the
    //                server on the free path. That is this layer's standing
    //                invariant, and the client already has the date it typed;
    //                it merges its own. On a re-access with no session the card
    //                prints no date, which is exactly what the retired sharecard
    //                did (`buildFooter` treats a missing date as a first-class
    //                case, not a gap). The PAID endpoint does supply it, because
    //                a buyer's hi-res card has to be correct on a fresh device.
    card: freeCardData(chart, semanticJson),
