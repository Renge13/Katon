...
// ============================================================
// UPCOMING_COPY — the two unbuilt products, and every string is UNRULED
// ============================================================
// Prompt Q commit 4. Reyner is the sole authority on Indonesian register and has
// NOT ruled this block's wording. Every reader-facing string for it lives in this
// one object so that ruling it is one file to open and ten values to replace,
// rather than a hunt through a 1100-line component.
//
...
//
// ── THE SENTINEL, AND WHY IT IS UGLY ON PURPOSE ──
// `PENDING()` wraps every unruled value in `@@UNRULED: ...@@`. It is deliberately
// unmissable: if one of these ever renders to a real reader, the bug reports
// itself in the screenshot instead of looking like a slightly odd Indonesian
// sentence that nobody questions. A placeholder that reads plausibly is the one
// that ships.
//
...
//
// COWORK PROPOSED WORDING ON 2026-08-29 and it is recorded in the PR body, NOT
// here. Putting an unruled proposal in the slot it would occupy is how a proposal
// becomes a decision by accident.
// ============================================================

/** Wrap an unruled string so it cannot be mistaken for a ruled one. */
const PENDING = (slot) => `@@UNRULED: ${slot}@@`;

export const UPCOMING_COPY = {
  /** The quiet eyebrow over the block. Secondary to the Artifact CTA, never a rival. */
  eyebrow: PENDING('eyebrow di atas blok produk berikutnya'),
  /** One line under the eyebrow saying these are not for sale yet. */
  lead: PENDING('kalimat pengantar, dua produk ini belum dijual'),

  compat: {
    label: PENDING('nama produk compat'),
    sub: PENDING('satu kalimat penjelas compat'),
  },
  annual: {
    label: PENDING('nama produk annual'),
    sub: PENDING('satu kalimat penjelas annual'),
  },

  /** The not-yet-available marker on each card. Reyner has an opinion on this one. */
  availability: PENDING('penanda belum tersedia'),
  /** The tap target. Tapping IS the metric; it must not read like a purchase. */
  interestCta: PENDING('ajakan untuk diberi tahu saat siap'),

  /** Shown only AFTER the tap has already been recorded. */
  thanks: PENDING('konfirmasi setelah ketuk, minat sudah tercatat'),
  /** The optional contact field. Optional must be visible in the label itself. */
  contactLabel: PENDING('label kolom kontak, boleh dikosongkan'),
  contactSubmit: PENDING('tombol kirim kontak'),
};
