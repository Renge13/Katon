...
// ── THE COPY-BANK REGISTRY ─────────────────────────────────
// A bank registers ITSELF here, at its definition site, and
// `lib/site/unruledScan.js` builds the gate's scan list from this object.
//
// The gate used to keep its own hand-written list of banks, and that list was
// wrong three times in three days - COMPAT_COPY on 2026-09-07,
// GLOSSARY.kompatibilitas on 2026-09-08, and before either of them the gate had
// never executed on Vercel at all (COWORK-BRIEF row 46). Every fix widened the
// list; none of them removed the defect, which is that **a gate enumerating its
// subjects by name is blind to the next subject**, silently, in a way that reads
// as a pass.
//
// `register` RETURNS its argument, so a bank is declared and registered in one
// expression and there is no second line to forget.
// `tests/unruled-copy.spec.mjs` reflects over this module and fails if any
// `*_COPY` export is missing from `COPY_BANKS`, which is what makes this
// structural rather than a fourth widening.
export const COPY_BANKS = {};

/** Register a copy bank under the name the gate should report. */
function register(name, bank) {
  COPY_BANKS[name] = bank;
  return bank;
}

export const SITE_COPY = register('SITE_COPY', {
  /**
  ...
  harga: {
    ...
    payment: 'Pembayaran memakai QRIS dan diproses oleh DOKU. Semua harga dalam rupiah, sekali bayar, tanpa langganan.',
  ...
  tentang: {
    ...
    operatorAfter:
      ', badan usaha yang terdaftar di Kota Tangerang Selatan, Banten. Pembayaran diproses lewat QRIS oleh DOKU. Untuk pertanyaan apa pun, termasuk soal datamu, kirim email ke',
  },
   ...
   * - EMAIL IS CAPTURED NOW, at the compatibility checkout, and two slots below
   *   disclose it. **This line used to read "no email is captured anywhere
   *   today, so none is claimed", and RULING C (Reyner, 2026-09-07) said in
   *   advance that it would go false the day the first compat checkout took
   *   one - "when that ships, /privasi changes in the SAME PR that ships it,
   *   not before".** It shipped in X-b3 commit 4 and this is that PR.
   *
   *   What is true, and it is narrow: the email is STORED and NOTHING is sent
   *   (ruled 2026-09-08, store-only for v1). It is an identity for access and
   *   recovery; there is still no password and no account. A disclosure that
   *   implied delivery would be false in the other direction.
   *
   *   AMENDED 2026-09-09: "stored, and used ONLY for recovery" was half the
   *   reason the field exists. The payment provider requires a payer email, so
   *   the address leaves Katon on every checkout, and both `privasi_email` and
   *   `PASANGAN_COPY.form_email_help` now say so. "hanya untuk mengakses
   *   kembali" and "tidak ... membagikannya" were the two clauses that stopped
   *   being true; the no-send promise is untouched, because it is still kept.
   * - THE SECOND PERSON'S BIRTH DATA IS STORED, and the reader who entered it is
   *   not that person. That is a disclosure a single-subject product never had
   *   to make, and it is why `privasi_second_person` exists rather than the
   *   birth-date line being widened: the fact is not "we store a birth date", it
   *   is "you gave us someone else's".
  ...
  privasi: {
    ...
    collect: [
      ...
      'Catatan pembayaran dari DOKU: nomor invoice, jumlah, dan status. Kami tidak pernah menerima atau menyimpan nomor kartu maupun data akun bankmu.',
    ...
    // ── TWO SLOTS ADDED 2026-09-08 WITH THE COMPAT SURFACE ────
    // Unruled, like every other reader-facing string that shipped with it, and a
    // production build refuses while they are. They are listed with the OTHER
    // collect items rather than in a section of their own: a reader looking for
    // what is stored should find all of it in one list.
    //
    // `privasi_email` AMENDED 2026-09-09 with `PASANGAN_COPY.form_email_help`,
    // in ONE commit, under the four-promise rule at the head of
    // `docs/content/pasangan-copy-rulings.md`: two strings describing the same
    // behaviour must never be able to disagree about it.
    privasi_email: 'Alamat email dipakai untuk pembayaran dan membuka kembali bacaan jika tautan hilang, tidak untuk yang lain. Kami tidak pernah mengirim pesan ke email tersebut.',
    privasi_second_person: 'Data tanggal lahir orang kedua disimpan khusus untuk bacaan ini. Data tidak dipakai untuk membuat profil terpisah dan akan ikut terhapus jika bacaan dihapus.',
    ...
    processors: [
      ...
      'DOKU, untuk pembayaran QRIS. Data pembayaranmu diproses di sistem mereka, bukan di sistem kami.',
  ...
  syarat: {
    ...
    paid: [
      ...
      'Pesananmu berlaku setelah pembayaran dikonfirmasi oleh DOKU. Sebelum konfirmasi itu masuk, belum ada pesanan yang berjalan.',
  ...
  pengembalian: {
    ...
    how: [
      ...
      'Bukti pembayaran atau nomor invoice dari DOKU.',
  ...
  // ── THE FRONT DOOR, ONE LINK (Prompt AA, 2026-09-24) ──────
  // ~~The two paths, 2026-09-08: home_mirror_label / _sub, home_compat_label /
  // _sub~~ DELETED with the two-card grid. `home_compat_link` is the one text
  // link under the button that replaces them.
  //
  // RULED 2026-09-24 by Cowork on Reyner's delegation (AA §4; rulings worksheet,
  // amendment k). Swept: 70 patterns, 0 hits; control fired 3. The arrow is
  // `Icon.arrow` beside it, never a character in the string.
  home_compat_link: 'Baca dinamika dua orang',
  // HOME ONLY. The lock line under the Home button, carrying "Gratis", which left
  // the page with the deleted mirror card. The compat stepper keeps its own
  // hard-coded `Privat. Hanya bisa diakses via tautanmu.`, because compat is paid.
  home_lock_line: 'Gratis dan privat. Hanya bisa diakses via tautanmu.',
  // (the four deleted slots' history, kept for the record:) Mirror (understand
  // yourself, free) and Compatibility (the dynamic between two people, paid).
  // Ruled 2026-09-07: neither is a prerequisite for the other. These four lived
  // in SITE_COPY rather than in PASANGAN_COPY so that
  // Home reads its own labels from ONE bank - a component pulling two strings
  // from one bank and two from another is how a pair of labels drifts apart.
});
...
export const COMPAT_COPY = register('COMPAT_COPY', {
  /**
...
// ── PASANGAN_COPY ─────────────────────────────────────────
// Every reader-facing string of the Compatibility surface, and every one of them
// ships as a PENDING() sentinel.
//
// ── WHY THE SLOTS EXIST BEFORE THE WORDS DO ────────────────
// Reyner is the sole authority on Indonesian register (rule 20) and he rules copy
// by SEEING it in place, on a phone. So the surface ships first with named holes,
// the preview build passes so it can be walked, and the production build REFUSES
// while any hole survives - which is the gate this bank registers into two files
// above. The alternative is Cowork or Code inventing Indonesian that then has to
// be un-invented, and the artifact's invoice description took three supersessions
// to settle by exactly that route.
//
// ── THE NAME IS INTERNAL AND THE ROUTE IS NOT ──────────────
// Reyner ruled the route `/kompatibilitas` on 2026-09-08. The bank stays
// `PASANGAN_COPY` and the slot prefix stays `pasangan_`: they are identifiers, a
// reader never sees either, and renaming them would churn every slot id in the
// rulings worksheet for no reader-visible gain.
//
// NO PRICE STRING. `price_note` is the sentence AROUND the number; the number
// itself comes from `lib/pricing.js#priceFor('compat')` at render time, because a
// price typed into a copy bank is a second source of truth for what a thing
// costs. UPCOMING_COPY's header makes the same rule and a spec enforces it on
// both.
export const PASANGAN_COPY = register('PASANGAN_COPY', {
  // ── the product block ──
  page_title: 'Bacaan Kompatibilitas',
  /**
   * The one-paragraph description under the title.
   *
   * REYNER'S STRUCTURAL BRIEF, recorded so the ruled string can be checked
   * against what was asked for: "A personalized reading of how your two patterns
   * meet - attraction, tension, elemental complementarity, day-to-day fit, and
   * the overall dynamic between you". That is the ORDER of P1..P5 in the engine,
   * which is why it is a brief and not a draft.
   *
   * AMENDED 2026-09-09, after Reyner walked the preview: the first ruling put
   * the whole brief on screen and read as a list. The brief is still the brief -
   * the engine order is unchanged - but a lead paragraph is not an index of it,
   * and three of the five carry the sentence now. The full five survive one
   * screen below in `includes_1..5`, which is the surface built to enumerate.
   */
  page_lead: 'Peta dinamika dua pola: tarikan, titik gesekan, dan ritme harian kalian.',

  // The five inclusions, in the reading's own journey order: P1 the two cores,
  // P2 the day seats, P3 the elements each supplies the other, P4 the pattern
  // badge, P5 the quadrant plus the P7 map.
  includes_1: 'Inti diri kalian berdua dan dinamika hubungannya',
  includes_2: 'Kursi pasangan di bagan masing-masing saat saling bertemu',
  includes_3: 'Elemen yang saling memengaruhi di antara kalian',
  includes_4: 'Pola hubungan: Cermin, Serumpun, atau Kontras',
  includes_5: 'Kuadran tarikan dan ritme, plus peta dinamika: poin penguat, area gesekan, dan komitmen masing-masing',
  price_note: 'Sekali bayar. Hasil tersimpan dalam satu tautan yang bisa dibuka kapan saja.',
  ...
  /**
   * Why the email is asked for.
   *
   * IT MUST BE TRUE, AND WHAT IS TRUE IS NARROW. Reyner ruled email STORE-ONLY
   * for v1 on 2026-09-08: it is kept for access and recovery, and NOTHING is
   * sent. A help string promising delivery would be the product saying something
   * untrue about itself, which is the same constraint UPCOMING_COPY.contactSent
   * carries at its own slot.
   *
   * AMENDED 2026-09-09, and the question came from Reyner on the walk: "what is
   * the email for?" The ruled string answered recovery only, which is the
   * SECOND reason. The first is that the payment provider requires a payer
   * email - that is why the field exists at all - and a help string that hides
   * the reason for a field is a different kind of untrue from one that
   * overclaims. `SITE_COPY.privasi.privasi_email` moved in this same commit.
   */
  form_email_help: 'Diperlukan untuk pembayaran dan untuk membuka kembali bacaan jika tautan hilang. Tidak ada email yang dikirim.',
  form_submit: 'Lanjut ke Pembayaran',
  /**
   * The season gate, asked about the OTHER person.
   *
   * The mirror's gate asks the reader about her own birth near a solar term. For
   * B it is a question about someone else, which is a different sentence and not
   * a pronoun swap.
   */
  season_gate_b_intro: 'Tanggal lahirnya bertepatan dengan pergantian musim. Jawab sebisamu, atau tanyakan langsung kepadanya jika ragu.',

  // ── after payment ──
  pending_title: 'Menunggu Konfirmasi Pembayaran',
  // RULED 2026-09-22 (Reyner), verbatim, launch-cut item 4 (source: the Cowork state
  // doc of 2026-09-22 §0, "pending_body contact line"; swept: 0 blocklist hits). It
  // replaces "Tidak perlu memuat ulang." with what to do when the page does NOT
  // update - the case the automatic check-status on load exists for, and the one
  // where a buyer who paid would otherwise have no way to reach anyone.
  pending_body: 'Halaman ini otomatis diperbarui setelah pembayaran diterima. Jika sudah membayar tetapi belum berubah dalam 5 menit, kirim tautan halaman ini ke hello@katon.app.',
  // `paid_title` ('Bacaan Kalian Sudah Siap') DROPPED 2026-09-09. It announced
  // the report to the one reader already looking at it, and it read like a
  // system notice above a piece of writing. Y-2 opens the report with
  // `page_title` and the engine's own pair line instead; the slot is deleted
  // rather than left unused so nothing can render it again by finding it here.
  // `link_keep` MOVED TO CHROME_COPY on 2026-09-09 (amendment g), removed here
  // and NOT aliased. It said "bacaan KALIAN" - the couple's - so the mirror's
  // copy-link box had to ship wordless in #113. The replacement has no plural in
  // it and serves both surfaces from one slot.
  /** Returning to a pair that was created but never paid for. */
  unpaid_resume: 'Bacaan ini belum dibayar. Selesaikan pembayaran untuk membukanya.',

  // ── the report: ONE EYEBROW PER SECTION ───────────────────
  // A small eyebrow over every report block; the big headline under it is the
  // glossary `name_id` of that block's primary fact, never a model heading and
  // never composed here (rule 14).
  //
  // ── WHY THE TWO OLD SLOTS WERE RENAMED AND NOT JOINED ─────
  // `report_badge_eyebrow` and `report_quadrant_eyebrow` carried exactly the
  // words Reyner ruled for `section_pattern` and `section_rhythm` on
  // 2026-09-09. Keeping both names would have put ONE ruled value in TWO slots,
  // which is the shape that reddened CI for a day when a forge test held its own
  // copy of the price ladder. So they are renamed in place, no aliases: one slot
  // per ruled value, and `PasanganReport.jsx` moved with them.
  //
  // FOUR OF THESE SIX RENDER NOTHING YET. `section_core`, `section_seat`,
  // `section_element` and `section_close` are wired in Y-2 (Addendum 2 item 2).
  // They land here now so Y-2 carries no copy edit at all - a feature commit
  // that also rules a word is a feature commit nobody can revert cleanly.
  section_core: 'Inti Diri',
  section_seat: 'Kursi Pasangan',
  // AMENDED 2026-09-09 (amendment f). It was 'Penyeimbang Unsur' - the same words
  // as `kompatibilitas.p3_supplies.name_id`, so P3 said its own label twice, one
  // line apart in two type sizes. Cowork's miss, recorded in the rulings file:
  // the eyebrow was swept against the blocklist and never against the block's
  // own name_ids.
  section_element: 'Keseimbangan Unsur',
  section_pattern: 'Pola Hubungan',
  section_rhythm: 'Tarikan & Ritme',
  section_close: 'Peta Dinamika',
  notfound_title: 'Bacaan Tidak Ditemukan',

  // ── SALES CLOSED, 2026-09-08: KATON IS EXITING XENDIT ─────
  // Reyner's drafts from the Y-2 table of docs/content/pasangan-copy-rulings.md,
  // shipping here rather than in Y-2 because Y-1 is what closes sales and a
  // closed shop needs something to say the moment it closes. They ship as
  // LITERALS rather than PENDING() under the 2026-09-08 chrome process ruling -
  // Compatibility UI chrome ships from Cowork's swept drafts and Reyner amends in
  // place - and a production build refuses on a sentinel, which would have made
  // the hotfix undeployable.
  //
  // "sementara" is load-bearing and it is true: DOKU is the next provider, this
  // is a pause and not a withdrawal. The second sentence is the one that matters
  // to somebody who has already paid, so it is in the body and not a footnote.
  ...
  // ── THE COMPAT PDF (prompt Y-3), FIVE SLOTS, ALL UNRULED ───
  // Reyner rules these in `docs/content/pasangan-copy-rulings.md` amendment i, and
  // Y-3's commit 4 applies his words verbatim. Cowork's proposals are recorded on
  // each slot so his ruling has something to amend rather than a blank.
  //
  // ── THEY SHIP AS PENDING(), AND THAT IS A DEPARTURE WORTH NAMING ──
  // The 2026-09-08 chrome process ruling above says Compatibility UI chrome ships
  // as swept LITERALS and Reyner amends in place, precisely so a production build
  // is not blocked by a sentinel. **Reyner overrode that for these five on
  // 2026-09-14**, in the same instruction that released Y-3: build around the slots
  // and fail the production build on any unruled one. Y-3 says the same thing and
  // names the precedent - `compat_invoice_desc` took three supersessions to settle
  // by the invent-then-un-invent route.
  //
  // The two are not in conflict once the reason is stated: 09-08 was about a
  // HOTFIX for a closed shop, where an undeployable build was the larger harm.
  // These five are unshipped copy in an unshipped document, so there is nothing to
  // hotfix and the sentinel costs nothing but the wait. `report_download_pdf` is
  // the one that is genuinely chrome; it is held with the other four because the
  // button and the document it produces should be ruled in one sitting.
  //
  // A `preview` build still passes, so the document can be walked and read on a
  // phone - which is how Reyner rules copy, and the whole reason the holes are
  // named rather than filled.
  /** Cowork proposal: `Bacaan kompatibilitas dari dua bagan kelahiran.` (swept clean). */
  pdf_cover_sub: 'Bacaan kompatibilitas dari dua bagan kelahiran.',
  /** Cowork proposal: `Bagan Kelahiranmu` (swept clean). Chart A, the reader's. */
  pdf_chart_a_heading: 'Bagan Kelahiranmu',
  /** Cowork proposal: `Bagan Kelahiran Dia` (swept clean). Chart B. */
  pdf_chart_b_heading: 'Bagan Kelahiran Dia',
  /** Cowork proposal: `Data di Balik Bacaan Ini` (swept clean). The facts table. */
  pdf_facts_heading: 'Data di Balik Bacaan Ini',
  /**
   * Cowork proposal: `Unduh PDF` (swept clean).
   *
   * NO EXISTING LABEL TO REUSE. Y-3 says to prefer the mirror delivery button's
   * label if there is one; there is not:
   *
   *     $ grep -rn "Unduh\|download" components/ lib/site/copy.js lib/render/copy.js
   *     -> only comments about downloaded PNGs and a CAPTURE_KINDS union
   *
   * so this is a new slot rather than a second copy of an old one.
   */
  report_download_pdf: 'Unduh PDF',
});

export const UPCOMING_COPY = register('UPCOMING_COPY', {
  ...
  // ── THE compat ROWS ARE GONE, 2026-09-08 ──────────────────
  // This block advertises what is NOT for sale. Compatibility IS for sale now -
  // it has a route, a price and a checkout - so leaving it here would tell a
  // reader the product she can buy is unavailable, one screen below the card
  // that sells it. `availability: 'Belum tersedia'` was ruled for this block and
  // still applies; it now applies to `annual` alone.
  //
  // The two ruled strings are deleted rather than moved. `Kamu dan Satu Orang`
  // and `Kecocokan, dibaca dari dua tanggal lahir.` were ruled for a
  // NOT-FOR-SALE teaser, and the front-door card is a different surface with a
  // different job; reusing them there would be Cowork deciding they still fit,
  // which is Reyner's call (rule 20). PASANGAN_COPY's slots ship unruled.
  annual: {
...
// ── CHROME_COPY ───────────────────────────────────────────
// Site chrome for Y-2: the persistent header, the compat stepper, and the
// report's own states. Ids are `chrome_*`; the bank is registered, so
// `scripts/check-unruled-copy.mjs` refuses a production build if any of these
// ever ships as a sentinel.
//
// ── THEY ARE COWORK DRAFTS, AND THAT IS A RULED PROCESS ────
// Under the 2026-09-08 chrome process ruling, Compatibility UI chrome ships from
// Cowork's swept drafts and Reyner amends IN PLACE after seeing it on a phone.
// So these land as literals rather than `PENDING()` sentinels: a production
// build refuses on a sentinel, which would make the very preview he has to walk
// undeployable. The drafts are the "Y-2 strings" table of
// `docs/content/pasangan-copy-rulings.md`, swept against the real guards.
//
// ── THE WHOLE BANK LANDS AT ONCE, ON PURPOSE ───────────────
// Every later Y-2 commit is then structure only, with no copy edit in it. That
// is the same reasoning that put `section_core`..`section_close` in
// PASANGAN_COPY one PR early: a feature commit that also rules a word cannot be
// reverted as either one.
//
// ── sales_closed_* IS NOT HERE, AND ITS ABSENCE IS THE POINT ──
// The Y-2 table lists `sales_closed_title` and `sales_closed_body`, but they
// shipped EARLY into PASANGAN_COPY with Y-1, because Y-1 is what closed sales.
// Copying them here would put one ruled value in two banks - the exact defect
// that renaming `report_badge_eyebrow` instead of duplicating it avoided in
// #112. The report reads them from PASANGAN_COPY.
export const CHROME_COPY = register('CHROME_COPY', {
  // ── the persistent header (commit 1) ──
  // The wordmark is a MARK, not a copy slot: `KATON.APP` is set in the component
  // as letterspaced type, the same object the footer already renders.
  nav_mirror: 'Bacaan Diri',
  nav_compat: 'Kompatibilitas',

  // ── the stepper (commit 2) ──
  // 1 Kamu, 2 Dia, 3 Email. `step_1` and `step_2` deliberately carry the SAME
  // words as PASANGAN_COPY.form_a_legend / form_b_legend, which are Reyner's
  // ruled 2026-09-08 values for the two form cards. They are not re-pointed at
  // that bank: the legends label a FIELDSET and these label a STEP, and the
  // stepper is what replaces those cards. When commit 2 lands, whichever pair
  // stops rendering should be deleted rather than left as a second copy.
  step_1: 'Kamu',
  step_2: 'Dia',
  step_3: 'Email',
  step_next: 'Lanjut',
  step_edit: 'Ubah',
  /** Shown in a completed step's summary when no hour was given. */
  summary_time_unknown: 'jam tidak diisi',

  // ── the report's states (commit 3) and the mirror's copy button (commit 4) ──
  copy_link: 'Salin tautan',
  /**
   * The URL field's accessible name (Y-2c item 1). The field is read-only and
   * shows the URL; a screen reader needs to be told what the value IS, and the
   * sentence above it is guidance rather than a label.
   *
   * Cowork draft under the 2026-09-08 chrome process ruling; Reyner amends in
   * place.
   */
  copy_link_field_label: 'Tautan bacaan',
  /**
   * The TOAST after a successful copy, not a button relabel (amendment g's
   * commit). Reyner confirmed these words for the toast.
   */
  copy_link_done: 'Tautan tersalin',
  /**
   * Shown when the clipboard REFUSES - no `navigator.clipboard` off HTTPS, or a
   * rejected write on an unfocused document. The URL is selected as the fallback
   * and this says what to do with it, because a selection with no message looks
   * like nothing happened, which is the complaint this whole item exists for.
   *
   * Cowork draft under the 2026-09-08 chrome process ruling, swept 0/70; Reyner
   * amends in place.
   */
  copy_link_fallback: 'Tautan sudah ditandai. Tahan untuk menyalin.',
  /**
   * ONE SENTENCE FOR BOTH SURFACES (amendment g). It lived in PASANGAN_COPY and
   * said "bacaan kalian", which is true of the compat report and false of the
   * mirror, so #113 gave the mirror the control with no sentence at all. Moved
   * here, not aliased: two slots holding one ruled value is the defect the
   * eyebrow rename avoided in #112.
   *
   * IT ALSO LEFT THE PROMISE FAMILY. The old wording carried "tanpa akun dan
   * tanpa kiriman email"; that claim now lives only in `form_email_help`,
   * `privasi_email`, `privasi_second_person` and the under-CTA line, and the
   * rulings file's promise list says so.
   */
  link_keep: 'Simpan tautan ini untuk membaca kembali.',
  ...
  /**
   * The error body names an address, and that is a promise about a mailbox.
   * `hello@katon.app` is `ENTITY.email`, which the footer already renders on
   * every route, so this is not a new claim - but if that address ever moves,
