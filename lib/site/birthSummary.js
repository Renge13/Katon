 ...
 *
 * MOVED HERE FROM `components/BirthFields.jsx` (AB §4, 2026-09-23), which
 * re-exports it unchanged. The PDF cover now prints `pairLine` too, and a PDF
 * composer runs under plain node, which cannot import a `.jsx` file - so the
 * words had to live where both the select and the document can read them. Still
 * ONE copy: the select's option labels and every summary read this object.
 */
