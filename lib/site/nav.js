...
/**
 * Which nav entries render. RULED 2026-09-23: while the payment fence is closed,
 * no paid entry point renders, and the compat link is one.
 *
 * `salesOpen` IS THE FENCE'S ANSWER, passed down from the server layout that read
 * `checkoutOpen()` - never recomputed here. A second copy of the rule is how the
 * header and the pay route would come to disagree.
 *
 * @param {{salesOpen: boolean}} opts
 * @returns {Array<'mirror'|'compat'>}
 */
export function navKeys({ salesOpen = false } = {}) {
  return salesOpen ? ['mirror', 'compat'] : ['mirror'];
}
