...
// lib/bazi/report/*, tenGods.js, mainProfile.js and tenGodTally.js all keep working
// untouched. (lib/chart.js and lib/readingView.js were on this list until
// 2026-08-23; both were retired with the contents/*.md path at the promotion.)
// ============================================================
 ...
 * @param {'male'|'female'|null} [params.gender] - Optional. Threaded through and
 *   persisted; affects luck-pillar direction ONLY, and luck pillars are not built.
 *   Never read by the natal computation.
 ...
 */
export function calculateBaziChart({ birthDate, birthTime = null, timezone = null, termSide = null, gender = null }) {
  const pillars = computePillars({ date: birthDate, time: birthTime, tz: timezone, termSide, gender });
  ...
  return {
    ...
    // 胎元 — DISPLAY ONLY, 5/5 against Joey. 命宮 is deliberately absent: no
    // candidate convention reproduces him (4/5 at best), and in a block whose
    // only job is to be cross-checkable a wrong value is worse than none. See
    // pillars.ts and docs/prompts/D1b-remove-life-palace.md.
