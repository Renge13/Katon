...
| V6 | **Round 3, the final round per the cap** (Reyner, 2026-09-24, `docs/prompts/AC-qris-walk-voice-round3.md` §B). Writer UNCHANGED (`gemini-3.1-flash-lite`, pinned; writer review deferred to after launch). Same v2 prompts, no edits. Fix (i): archetype bracket from `core.archetype_name_en`. Fix (ii): typography normalised post-write (dashes, curly quotes, ellipsis, hanzi-only brackets). Each accept-changing edit its own commit and bump. J1 as a provisional hard gate, judge unchanged (Pro), calibrated first: every seeded invention caught in 3 runs AND zero clean J1 false positives, one fix round allowed, else advisory for MVP. J2-J4 advisory; no writer constraint from any judge finding | built as ruled: fix (i) `b1fdc80` (1.31.0), fix (ii) `6fa2a8f` (1.32.0), J1 gate `f36e6da` (1.33.0). One flag: the ruling said (ii) was "same as v1", but v1 has no typography normaliser (it rejects these characters and regenerates), so (ii) is new v2-only code | **ruled 2026-09-24. J1 OUTCOME: PASS on the first calibration** (`a22bc94`, `docs/qa/2026-09-25-voice-v2-j1-calibration.md`): 15/15 seeded caught, 0 clean J1 false positives, no fix round used. **J1 is HARD from 1.33.0.** Round-3 renders: `docs/qa/2026-09-25-voice-v2-round3-renders.md` |
