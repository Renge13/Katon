...
UPDATED: 2026-09-09 — SITE CHROME (Y-2). A persistent header on every route; the compat form is a
         stepper with the season gate inside its own step; the paid report has seven NAMED states and
         a way out of each; every report heading is the engine's, none the model's. Three defects
         closed that were each a state or a heading wearing another's clothes: a dropped connection
         rendered the UNPAID product block to someone who had already paid, the render wait said
         "Menunggu Konfirmasi Pembayaran", and the first frame was blank. The hour input became a
         24-option select. `npm test` now runs everything CI runs - it was missing `report:forge`,
         which BLOCKS A MERGE, so a local 61/61 could sit in front of a red CI.
UPDATED: 2026-09-09 (evening) — REYNER'S PHONE WALK OF Y-2, AND THE FIXES (Y-2b). Confirmed GOOD by
         him: no blank frame after the CTA, the hour picker, the skeleton, the cross-fade, the header
         navigation. FIXED: the logomark moves into the header and the page keeps only one; the copy
         button COPIES AND TOASTS instead of highlighting the URL; `link_keep` becomes one
         plural-free sentence in CHROME_COPY serving both surfaces; `section_element` becomes
         "Keseimbangan Unsur"; the P0 opening stops naming three people.
         **AND THE BIG ONE: Y-2's names projection shipped EMPTY.** It read `f.entry?.name_id`, a
         field `fact()` never emits, so P1/P2/P3 rendered an eyebrow with no headline on every paid
...
UPDATED: 2026-09-21 (Prompt V) — THE DOKU CHECKOUT ADAPTER IS BUILT, AND QRIS IS SWITCHED OFF ON THE
         ACCOUNT. `PAYMENTS_PROVIDER` becomes `doku | mock | closed`; `lib/doku/{signature,client,
         notify}.js` and `POST /api/doku/notify` are live on the preview; the fence refuses DOKU
         without keys by NAME and refuses `DOKU_SANDBOX` in production (the subtler free unlock - the
         page looks real, the notification is genuinely signed, `paid` flips, and nothing was paid).
         Reyner's six `lib/site/copy.js` strings now name DOKU. **SALES ARE STILL CLOSED** and the
         production flip is a separate, later instruction; production was off limits for the whole
         prompt.
         **THE BLOCKER IS AN ACCOUNT SETTING, NOT CODE:**
           $ npm run probe:doku
             {"message":["PAYMENT CHANNEL IS INACTIVE"]}     # payment_method_types: ["QRIS"]
           $ (same account) payment_method_types: ["VIRTUAL_ACCOUNT_BCA"]
             200 {"message":["SUCCESS"],"response":{"payment":{"url":"https://staging.doku.com/..."}}}
         So credentials, signing and Checkout all work; only the QRIS channel is off. DOKU has to
         enable it. DEFERRED REGISTER carries the row.
         **THE SIGNING RULE IS CONFIRMED AGAINST THE SANDBOX, which §4 wanted and could not get from
         the docs:** the same request signed with a deliberately WRONG secret returns
         `{"code":"invalid_signature"}`, so DOKU verifies the signature BEFORE the body and a
         business-level answer proves the signature passed.
         **TWO FINDINGS THAT WOULD HAVE COST A REAL BUYER.** (1) `order.amount` arrives as the STRING
         `"39000"`; `amountMatchesSku` takes a number and returns false for a string, so a verified,
         correct, fully paid notification would have settled as `amount_mismatch`. (2) There are TWO
         error envelopes - `{"error":{"code","message","type"}}` with a string for auth failures, and
         a top-level `{"message":[...]}` ARRAY for every business answer including success. The
         amendment said "string, not array"; both are true, of different envelopes.
         **AND ONE TEST THAT DID NOT GUARD WHAT IT CLAIMED:** `A WRONG REQUEST-TARGET IS REJECTED`
         stayed GREEN when the handler was changed to derive the target from the request's own path,
         because that request's path already IS `/api/doku/notify`. Replaced by `THE TARGET IS THE
...
| **The front door. ONE THING TO DO since Prompt AA (2026-09-24)** | The headline, the sub-line, the birth form, ONE button (`Lihat Refleksiku`) and the lock line - and under it, only while the payment fence is open, ONE quiet text link to `/kompatibilitas` (`SITE_COPY.home_compat_link`). ~~The two-card grid (Bacaan Diri / Kompatibilitas, 2026-09-08)~~ is deleted, and with it the word "Gratis" (it lived in `home_mirror_sub`). ~~`home_compat_link` is an unruled sentinel~~ **RULED 2026-09-24 (amendment k): the link reads `Baca dinamika dua orang`, and "Gratis" returns in Home's own lock line, `Gratis dan privat. Hanya bisa diakses via tautanmu.` (`SITE_COPY.home_lock_line`, home only; the compat stepper keeps its plain lock line)** | `components/Funnel.jsx#Home`; `salesOpen` from `app/page.js` -> `checkoutOpen()` | none (the link: `checkoutOpen()`) |
...
| **Complete Edition, Rp 19.000** | Card B at download resolution (907x1747, no shadow, alpha corners) **and** the Complete Edition PDF - ~~cover, reading verbatim from `render_cache`, chart page with `hal. N` cross-references, appendix of every mechanic in her chart, colophon~~ **SINCE PROMPT AB (PR 1 #126, PR 2 `feat/ab-paid-layout`, 2026-09-23):** a composed cover (wordmark, `Edisi Lengkap` eyebrow, Indonesian archetype title with the English name once under it, the pillars as a quiet motif, `birthSummary` birth line, and the disclaimer + provenance at its foot); the reading verbatim at a ~70-character measure with Spectral headings; ONE chart page (four pillars + the conception pillar as a fifth cell, element presence drawn as bars); the two-column appendix (150pt term column). `Katon - Edisi Lengkap` runs in the footer of every page after the cover. No page numbers, no cross-references (R6) | `lib/deliver/handlers.js` -> `lib/pdf/build.js` (page-map fixed point, ship-blocking) + `lib/card/cardData.js` (full, with `appendix`) | **PAYWALL.** `row.paid === true`, flipped only in the verified Xendit webhook. Offered AFTER the reading, never in front of it |
| **The compat PDF. INCLUDED in the compat price** | ~~A 10-11 page document: cover, the reading verbatim from the cache row, both birth charts with their pillars and references, a facts table of every pair fact with its `Kamu`/`Dia` sides, and the legend of every glossary term the pair touches.~~ **SINCE PROMPT AB (2026-09-23), about 8 pages on the floor fixture:** a composed cover (`Bacaan Kompatibilitas` eyebrow, `Matahari dan Gunung` title, the English pair once, `pairLine` births, disclaimer + provenance at the foot); the reading with the quadrant as its title (C1); BOTH charts stacked on ONE page, each with its own bars; the facts table (`子 Tikus` on one line, muted description under each row, the palace-frame sentence once as a lead line, pair-level rows full width with the quadrant closing the table); the two-column legend. `Katon - Bacaan Kompatibilitas` in every footer after the cover. `GET /api/pair/[id]/pdf`, behind the bearer and the paywall, `attachment`, `private, no-store`. **IT NEVER RENDERS**: no cache row means 409 `reading_not_rendered`, so a floored pair gets a refusal and her next page load retries the provider. ~~**NOT YET REACHABLE IN PRODUCTION** - four of its five copy slots are unruled~~ **CORRECTED 2026-09-21: THE SLOTS ARE RULED AND THE PRODUCTION BUILD NO LONGER REFUSES.** The row was stale, not wrong when written. Measured on main:
`node -e "...strip comments...PENDING\(..."` -> **0** live `PENDING(` calls in `lib/site/copy.js` (six textual hits, all of them prose ABOUT the mechanism); and `npm run check:unruled` -> `OK No unruled copy in 6 bank(s)`. What still gates this row is the same thing gating everything paid: `PAYMENTS_PROVIDER` is unset in production, so no one can buy a pair to have a PDF of. The COPY reason is discharged | `lib/pair/servePdf.js` -> `buildPairPdf` -> `lib/pdf/pairDocument.js`, through the mirror's own builder | Stage 6 already passed: the document prints a cache row and validates nothing |
...
| **Compatibility. SALES CLOSED 2026-09-08, and the report a customer already paid for was BLANK** | **WHAT HAPPENED FIRST:** Reyner paid Rp 39.000 on production for pair `g4WH4_9QbCrCj3Gha934q` and received six headings, no paragraphs, the raw keys `contrasting` and `q4` as text, and only the penutup as prose. Both defects shipped in #105 and are fixed in Y-1: `ProseBlocks` now derives paragraphs from `text` (the mirror worked only because `lib/mirror/view.js:206` split on the way out), and the badge and quadrant NAMES are resolved on the server from the glossary, never sent as keys. The fixture is the verbatim production response. **WHAT IS TRUE NOW:** `PAYMENTS_PROVIDER` is `closed` in production - Katon is exiting Xendit and sales stay closed until DOKU. `POST /api/pay` answers 503 `payment_closed`; `/kompatibilitas` keeps its Home card and renders its sales-closed state; **existing paid pairs and paid mirror artifacts keep serving.** The paid flow is walkable for free with `PAYMENTS_PROVIDER=mock` (Preview and local only, refused when `VERCEL_ENV=production`), documented in `docs/ops/paid-flow-walk.md`. The webhook now warms the render on settle, so the buyer's first GET is a cache hit rather than a synchronous Gemini call, and every waiting state shows the report's own skeleton instead of a blank page | `lib/paymentFence.js#paymentsProvider` -> `POST /api/pay` / `POST /api/mock-pay/<id>` -> `settlePair` -> `GET /api/pair/<id>[/reading]` | **NO REAL XENDIT TRANSACTION BY ANYONE, including for testing** (Reyner, 2026-09-08). Xendit test keys are not needed and are not set; Reyner handles the balance and unsets the live keys. An unset `PAYMENTS_PROVIDER` means CLOSED, so a lost env var cannot silently reopen sales. **The 28 RULED copy strings are landed on main and NOT yet applied** - the bank still holds the drafts, and `tests/pasangan-copy.spec.mjs` reads the file's own STATUS to tell the two states apart |
...
| **The upcoming block itself. ANNUAL ONLY since 2026-09-08** | ~~Both products shown together~~ **Compat left this block when it went on sale** - a block that advertises what is not for sale cannot carry the product a reader can buy one screen above. `annual` alone, keeping the ruled `Belum tersedia`. Originally: both products shown together BELOW the Artifact offer and visually secondary to it - a text link, not a second Button - per the ruled order Mirror -> Artifact decision -> Compat / Annual interest. **EVERY READER-FACING STRING IN IT IS A VISIBLE `@@UNRULED: ...@@` PLACEHOLDER**, because Reyner rules Indonesian register and had not ruled this block; the structure shipped so commits 5 and 6 were not blocked behind a wording decision. The two PRICES are real and resolve from `lib/pricing.js` | `lib/site/copy.js#UPCOMING_COPY` -> `components/Funnel.jsx#Upcoming` | ~~**A PRODUCTION BUILD IS REFUSED WHILE ANY PLACEHOLDER SURVIVES** (`scripts/check-unruled-copy.mjs`, wired as `prebuild`)~~ **FALSE FROM 2026-08-31 TO 2026-09-08, AND NOW TRUE.** The gate was real, the wiring was half-real, and **it never ran on a single Vercel deploy**. `prebuild` is an npm LIFECYCLE hook: it fires before `npm run build` and not before `next build`, and `vercel.json` said `"buildCommand": "next build"` from `8953008` (2026-07-07) - which predates the gate by eight weeks, so it was bypassed from the moment it was written (`7cec498`, 2026-08-31). Corrected in `fix/production-copy-gate` (2026-09-08); `"buildCommand": "npm run build"` now, and a local `VERCEL_ENV=production npm run build` prints `> katon@0.0.0 prebuild` then `OK No unruled copy in 2 bank(s)`. **HOW IT WAS FOUND, because nothing in CI could have:** on 2026-09-07 a compat invoice string shipped as a deliberate `@@UNRULED@@`, the PR said production would refuse the build, and the build succeeded and served it. **Preview and local builds still pass deliberately** - the block has to be seen in order to be ruled. See COWORK-BRIEF section 4 |
...
| **Site chrome. NEW 2026-09-09 (Y-2)** | A persistent HEADER on every route: the wordmark to `/`, and two links naming the two products, with the current one marked in ink AND in `aria-current`. Mounted in `app/layout.js` beside the footer, so no page can omit it - `app/` has exactly one layout and a test walks the route table to keep that true | `components/SiteHeader.jsx` + `lib/site/nav.js` + `CHROME_COPY` | none |
| **The compat form. A STEPPER since 2026-09-09** | ~~Three identical cards on one screen, six fields for two people before anything acknowledged her~~ **1 Kamu, 2 Dia, 3 Email, one open at a time.** A completed step collapses to a one-line receipt - `13 Sep 1989, 09.00, Laki-laki` - with `Ubah` to reopen it; no hour SAYS SO, no gender is silent. **The season gate now opens INSIDE the step it belongs to** rather than taking over the page at checkout, which is what stops two boundary births being asked back to back with only an intro sentence telling them apart | `components/PasanganSteps.jsx` + `lib/site/birthSummary.js` | the paywall is unchanged; this is all pre-payment |
| **The hour picker, MIRROR AND COMPAT** | ~~`<input type="time" step="3600">`~~ **a 24-option select.** `step` is a validation hint, not an input mode: browsers still rendered a minute field and merely marked 09:30 invalid, so the form asked for precision the submit handler then discarded. Stored value unchanged (`HH:00`), displayed `09.00`. Measured: truncating every fixture minute to the hour changes **0 of 13** Joey charts | `components/BirthFields.jsx#HOURS` | none |
| **The paid report. SEVEN NAMED STATES since 2026-09-09** | not found · unpaid · pending payment · rendering · ready · floor · error. **A dropped connection used to render the UNPAID product block** - offering to sell a reading to someone who already owned one - because a failed fetch and a 404 were indistinguishable. **The `rendering` wait used to say "Menunggu Konfirmasi Pembayaran"** to a reader whose money was already taken. **The first frame used to be blank** for as long as two fetches took, right after a checkout redirect. All three closed. Every state has a way out, and a poll that runs out is a visible error rather than a skeleton that animates forever | `lib/pair/reportView.js#viewFor` -> `components/PasanganReport.jsx` | `row.paid === true`, unchanged |
| **The report's headings. ENGINE-OWNED, ALL OF THEM** | Every block: a ruled section eyebrow (`Inti Diri`, `Kursi Pasangan`, `Penyeimbang Unsur`, `Pola Hubungan`, `Tarikan & Ritme`) over the glossary `name_id` of its primary fact. **NOT ONE model-written heading survives** - the model titled blocks after their own badge, so `Pola Kontras` appeared three times in three lines. The header above them is `page_title` plus one line naming the pair from facts: `Laki-laki, 13 September 1989 dan Perempuan, 4 Maret 1990` | `serveReading` projects `names` from every fact's own glossary cell -> `ProseBlocks` with `modelHeadings={false}` | behind the paywall |
| **The link, on BOTH products** | The URL with a COPY button beside it. It selects the text first, always - that is both the fallback and the feedback, because `navigator.clipboard` is undefined off HTTPS and rejects when the document is unfocused, and Katon's reader is on a phone inside somebody's in-app browser. **The mirror's box carries no sentence**: `link_keep` says "bacaan kalian" and the mirror's reading is hers alone | `components/CopyLink.jsx` | none |
...
|---|---|---|---|---|---|
| **PAID CTA HIDDEN WHILE FENCE CLOSED, ruled 2026-09-23, lifts on DOKU flip** | While `lib/paymentFence.js` answers closed, NO paid entry point renders: the Rp 19.000 Complete Edition offer at the foot of the free reading, the compat card on the home page, and the compat link in the header. The free reading only. A buyer who already paid still gets her delivery, because closing sales is not revoking a purchase. Any route still reachable by an old link or the back button answers `payment_closed` and the client renders `PASANGAN_COPY.sales_closed_title` / `sales_closed_body` rather than a spinner. | **The CTA hung in production.** Tapping Rp 19.000 on katon.app showed a loading icon forever. Measured 2026-09-23: `POST https://www.katon.app/api/pay/<any id>` returned `503 {"error":"payment_closed"}`, and `Offer.startCheckout` in `components/Funnel.jsx` went to `stage = 'pending'` whatever the response said, so `Pending` drew `.k-spin` with no way out. Reyner ruled that hiding beats explaining while there is nothing to sell. | `PAYMENTS_PROVIDER=doku` with both DOKU keys set in production, which makes the fence's own `checkoutOpen()` true. **The CTAs come back with no code change** (a redeploy picks up the env value). Nothing in this row is deleted at that point: the hiding is conditional on the fence, not removed code. | Reyner, at the DOKU flip: open katon.app and see the offer under a fresh reading. | **OPEN**. Ruled 2026-09-23. |
...
| **PRODUCTION AND PREVIEW SHARE ONE SUPABASE DATABASE. Found 2026-09-25 (Code), ruled the same day (Cowork, rule 9; Reyner may overrule)** | **WHAT IT IS.** Every Preview deployment reads and writes the production database. Evidence (2026-09-25): reading `eJm6p6PjG8f_0eridE39x` was created on the `ops/doku-walk` PREVIEW and paid through a SANDBOX DOKU payment, and `GET /api/deliver/eJm6p6PjG8f_0eridE39x` returns it `paid:true` on production, on both `www.katon.app` and `katon-eta.vercel.app`. The polling Reyner saw on `katon-eta.vercel.app` from 13:51:58 WIB was Code's own browser tab: DOKU's auto-redirect sent the checkout page to Preview's `NEXT_PUBLIC_BASE_URL`, which is `katon-eta.vercel.app`. **WHAT IS UNGUARDED, and it is rule 18.** `POST /api/mock-pay/<id>` flips `paid` on ANY reading or pair it can find, with no invoice required, on any build where `mockPaymentsAllowed()` was true when it was built. With one database, that includes rows created on production. Probed read-only 2026-09-25 11:47Z, with an id that does not exist so both lookups miss: **25 live branch aliases** answer `404 {"error":"not_found"}` (the door ran its lookup), 4 answer `503 mock_payments_disabled`. So does a deployment's own permanent URL (`katon-40sxaxzg3-...`, the build of `d931aa7`). By GitHub's deployment records, **76 Preview deployments** were built between 2026-09-08 and the change, 16 of them in the 09-21/22 window when Preview was `doku`. Not exercised against a real row. Also: test rows sit among real ones and are not marked (§ KNOWN TEST ROWS below) | Nobody chose it: the Preview Supabase variables point at the production project. **Accepted for now by the Cowork ruling below**, because a separate database is infrastructure work and its trigger is set. **THE RULING, 2026-09-25:** (1) **DONE**: Reyner changed the general Preview `PAYMENTS_PROVIDER` from `mock` to `closed` today. It reached builds from then on: the rulings-branch build of 07:24Z still has the door open, the main build of 11:45Z has it closed. Mock walks from now on use a BRANCH-scoped `mock`, as `ops/doku-walk` does for `doku`. (2) ~~The `ops/doku-walk` branch variable (`PAYMENTS_PROVIDER=doku`) is deleted after Reyner's two production purchases.~~ **SUPERSEDED 2026-09-25 (Cowork, rule 9): deleted on flip day BEFORE sales reopen, under any gate-i option** (`docs/ops/doku-walk.md` § GATE i). The purchases run on production and never use that preview. (3) A separate Preview database goes into THE DEFERRED REGISTER | **A separate Preview database** (DEFERRED REGISTER, trigger: Prompt U or the first promotion beyond Reyner's own network, whichever comes first). **The mock door on existing deployments is NOT closed by the env change**: env values reach only new builds (why `ops/doku-walk` needed its empty commits, `docs/ops/doku-walk.md` § WALK 4). Closing those is a separate act of Reyner's | **Reyner** (env, Vercel, Supabase). Code re-runs the read-only mock-door probe on request | **OPEN.** (1) done, (2) is a flip-day step before sales reopen (gate i), (3) deferred. The pre-change deployments are still live as of 2026-09-25 11:47Z. **Closing the unguarded paths is PRODUCTION-FLIP GATE ITEM i** (Reyner, 2026-09-25): backlogged until before release, chosen on flip day from three options. The item, its options and its verification live ONLY in `docs/ops/doku-walk.md` § GATE i |

### KNOWN TEST ROWS IN THE SHARED DATABASE (listed 2026-09-25, so purchase counts can exclude them)

**Known, not exhaustive**: every Preview walk and every QA run against a deployment wrote rows here. The
in-memory harnesses (`scripts/qa-voice-v2-renders.mjs`, `probe:reconcile`) wrote none. Paid state read
2026-09-25 on production with `GET /api/deliver/<id>` (readings) and `GET /api/pair/<id>` (pairs).

| id | kind | paid | what it is |
|---|---|---|---|
| `eJm6p6PjG8f_0eridE39x` | reading | **yes** | Walk 4: SANDBOX DOKU QRIS, invoice `.muglimjz`, settled by reconcile-on-load. No money |
| `Pez1Ggq5SL311dsTIEtc6` | reading | **yes** | The Complete Edition PDF QA reading behind `reports/pdf-passable/` |
| `g4WH4_9QbCrCj3Gha934q` | pair | **yes** | **Reyner's own Rp 39.000 production purchase** (Xendit, before sales closed on 2026-09-08; LIVE STATE, Compatibility row). Real money, but a self-test: this file already counts it as a smoke test, not demand |
| `JoWcjAT0Rc3DlLQlj3JiT` | pair | **yes** | VA walk 1, 2026-09-21: sandbox VA paid; flipped by a hand-signed notification |
| `PZ0t_B3YDnzdXc2LWV38D` | pair | **yes** | The #126 Preview walk (`docs/launch-cut` alias) |
| `rVe4ca-FOhsprfGUucTxA` | pair | **yes** | The #122 Preview (`GET /api/pair/<id>/pdf` check) |
| `IKWSWkN5TKqBiOse6hHAu` | reading | no | 2026-09-25: carries a `mock_` invoice from the stopped first walk attempt |
| `qvW0XL5L2Zb25p9Xg71Uv` | pair | no | VA walk 2, 2026-09-21 (sandbox VA paid at DOKU, never flipped) |
| `JE8fscaWVJjsYLl7FGMRR` | pair | no | VA walk 3, 2026-09-21 (same) |
...
account. The partial tells: an `invoice_id` of `mock_<id>` was never money. A paid row with NO
`invoice_id` never went through a real checkout (`/api/mock-pay` sets `paid` without writing an
invoice). A DOKU invoice (`<id>.<suffix>`) has the SAME format on sandbox and production, so the only
authoritative test is asking PRODUCTION DOKU for that invoice's status: a sandbox invoice is unknown
there. Until production DOKU keys exist, every DOKU-paid row is a sandbox row.
...
| **A DESIGN PASS ON THE FRONT DOOR AND THE COMPAT SURFACE - added 2026-09-09** | Reyner, after walking Y-2 on his phone: a **"dedicated design pass after the core product is done"**, and specifically that the home compat card **"looks weird"**. Y-2 and Y-2b added STRUCTURE - header, stepper, section headings, states - and deliberately no new visual language, so what is unaddressed is composition and hierarchy rather than a defect list. **WHAT IS UNGUARDED:** nothing in CI has an opinion about how a page LOOKS. jsdom has no layout and no stylesheet, the card audits cover the card's own contrast and budget and nothing else, and every "it renders" assertion in the suite is satisfied by an ugly page. So this row is the only record that the surface has not been designed, as opposed to built | **NOT BEFORE Y-3 / DOKU / a real-money walk** (Reyner's own ordering: after the core product is done). It closes when he has walked a deliberate design revision, not when a commit lands - there is no test that can close it |
| ~~**`?dari=<token>` DOES NOT PREFILL COMPAT STEP 1 - added 2026-09-09**~~ **CLOSED 2026-09-21** | The mirror-to-compat link carries a reading token and prefills NOTHING. Not a bug: `GET /api/mirror/<token>` returns no birth data at all, because the free card is built with `birthDate: null` precisely so nothing about a birth leaves the server on that path. Y-2's prompt asked for "prefills step 1 and starts on step 2"; it cannot be built from that payload, and `tests/pasangan-steps.spec.mjs` pins the honest behaviour ("`?dari` PREFILLS NOTHING, BECAUSE THERE IS NOTHING TO PREFILL") so the sentence cannot be implemented later without meeting it. **WHAT IS UNGUARDED:** friction on the MONEY path. A reader arriving from her own free reading retypes her own birth date before she can buy, and nothing measures how many stop there - the conversion counters two rows above are the same gap | **A CLIENT-SIDE `sessionStorage` WRITE at funnel submit, read on step 1 mount.** No payload change, no server change, no birth data crossing a network boundary it does not already cross - which is what keeps it compatible with the reason the payload is empty. **Cowork's advice: before promotion**, because it is on the paid path and cheap. **DONE 2026-09-21, exactly as prescribed:** `lib/site/carryBirth.js` writes at funnel submit and `Pasangan` reads in its state initialiser. No payload change, no server change, nothing crossing a network boundary it did not already cross. **AND THE ORIGINAL ASSERTION SURVIVES UNCHANGED** - `?dari` still prefills nothing, because the carry is `sessionStorage` and the link still holds only a reference; a shared link, a new device or a fresh session gets the empty form it always got. The handoff is driven end to end through both real components in `tests/carry-birth.spec.mjs`, because two sides agreeing on a key is the whole failure mode and neither side can be tested for it alone. **WHAT IS STILL NOT BUILT:** the other half of Y-2's sentence, "starts on step 2". A prefilled value she never saw is a claim about her she did not make, on the form that takes her money - and skipping her past it is a change she can SEE, so it is Reyner's (rule 9) and not a commit's. **The conversion counters two rows above are still the gap:** this removes the friction and still cannot measure how many were stopping there |
...
| **QRIS IS INACTIVE ON THE DOKU ACCOUNT - added 2026-09-21, and it is the only thing between Katon and revenue** | `POST /checkout/v1/payment` with `payment_method_types: ["QRIS"]` returns `{"message":["PAYMENT CHANNEL IS INACTIVE"]}` on the SANDBOX account (`npm run probe:doku`, 2026-09-21). It is an ACCOUNT SETTING, not code: the same account returns a real Checkout session and a `payment.url` for `VIRTUAL_ACCOUNT_BCA`, and a deliberately wrong secret on the same request returns `invalid_signature` instead - so the credentials, the signing rule and Checkout itself are all confirmed working, and only the channel is switched off. **WHAT IS UNGUARDED:** nothing in code. The adapter is built and tested; it simply has no channel to sell through. Note that `NOT_A_REAL_CHANNEL` returns the SAME message, so DOKU does not distinguish an unknown enum from a disabled one - the reading rests on the docs confirming `QRIS` is a valid enum plus VA working on the same account | **DOKU, not Katon.** Reyner asked by email per the 2026-09-08 notes; the answer decides whether §5's walk can happen at all. Production needs the same switch before any real sale, separately. Until then the §4 captured-QRIS-notification fixture cannot exist and `tests/doku.spec.mjs` says so in its own header. **A labelled VA capture would close everything except the QRIS-specific body fields** - it needs Reyner's yes, because Amendment 1 §B.5 says stop and report rather than improvise around an inactive channel |
| **INBOUND `Request-Timestamp` IS NOT CHECKED on `/api/doku/notify` - added 2026-09-21** | Amendment 1 §B.3(b), recorded rather than built, and the recommendation is DO NOT BUILD IT. **What it would guard:** replay of a signed SUCCESS - which is already idempotent, because `markPairPaid` only transitions false to true and `recordEvent` fires on the transition only (`tests/doku.spec.mjs`, "A REPLAYED SUCCESS IS IDEMPOTENT"). **What it would cost:** DOKU retries a failed delivery nine times over seven days, and a legitimate retry on day two is indistinguishable from a replay, so a one-hour window would REJECT real settlements. OUTBOUND is a different question and is already handled: DOKU enforces +-3600s on requests we send, checked before `Client-Id`, so `client.js` builds the timestamp at send time and never at module scope | **A reason to distrust idempotency**, which would be a change to `settlePair` rather than to the route. Nothing on the horizon |
| **A REPEATED `invoice_number` IS UNTESTED AGAINST DOKU - added 2026-09-21** | §3 said to DROP the `.${Date.now().toString(36)}` suffix if the sandbox showed DOKU returns the existing session on a repeat. **The probe could not answer**: all four calls failed on `PAYMENT CHANNEL IS INACTIVE`, for a reason unrelated to the question. The suffix stays because the question is UNANSWERED, not because it was answered - and `probe:doku` printed a confident "REJECTED. Keep the suffix" verdict off those four failures, which is an instrument reporting a conclusion where it learned nothing. That verdict line is the thing to fix before the next run. **WHAT IS UNGUARDED:** nothing. The suffix is the conservative choice either way, and `tests/doku.spec.mjs` asserts a BARE invoice number still finds its row, so dropping it later needs no change to the notify side | **Re-run `npm run probe:doku` once QRIS is enabled.** One run answers it. **ANSWERED 2026-09-25**: QRIS is active on sandbox and the re-run succeeded on every call. The same `invoice_number` twice returned two DIFFERENT `session_id`s (`ec1a2d9a...`, `ca1c046c...`), so DOKU creates a second live session and the suffix stays for a measured reason now (`docs/ops/doku-walk.md` § GATE b, 2026-09-25). The row can close |
| **THE `--live` FORGE CHECK `unpaid /full returns teaser only` TARGETS DELETED ROUTES - added 2026-09-21, PRE-EXISTING** | It fetches `/api/reading/<token>/full`, and the whole `app/api/reading` tree was deleted at the promotion (`a9024c7`). Against the preview it gets a 404 HTML page and fails on `Unexpected token '<'`. **It has been broken since 2026-08-23 and nobody saw it, because CI runs `report:forge` WITHOUT `--live`** - so the suite is green in CI and red the moment anyone actually points it at a server. Found while adding the two DOKU forgery checks beside it, which DO pass against the live preview. **WHAT IS UNGUARDED:** the proposition it was making - that an unpaid reader gets no paid content - is now `lib/deliver/handlers.js`'s and is covered by `tests/deliver-route.spec.mjs`, so this is a dead instrument rather than a hole | **Repoint it at `/api/deliver/<id>` or delete it.** Out of Prompt V's scope: it is not a payments change and rule "gate changes ship isolated" applies to the instrument as much as the gate |
| **THE NOTIFICATION BODY SHAPE IS UNVERIFIED AGAINST REAL DOKU BYTES - added 2026-09-21, and it is a PRODUCTION-FLIP GATE, no longer a merge gate** | V-doku.md §4 and Amendment 1 §B.6 require the signature fixture to be a CAPTURED Checkout notification. **It does not exist and could not be made to exist**: three sandbox walks produced three payments DOKU recorded as SUCCESS and ZERO notification attempts (Notification Center, read by Reyner), through both a channel-registered URL and a documented `additional_info.override_notification_url` that DOKU echoed back and then ignored. **WHAT IS UNGUARDED, precisely:** the notification BODY SHAPE - that `order.invoice_number`, `order.amount` and `transaction.status` sit where `lib/doku/notify.js` reads them, in a real Checkout QRIS notification. **WHAT IS NOT UNGUARDED, and the distinction is the whole row:** the SIGNING RULE is confirmed against the live sandbox (a wrong secret returns `invalid_signature` where a right one returns a business answer, so DOKU checks the signature before the body and an accepted request proves an accepted signature); and the ROUTE is confirmed end to end on a real Vercel lambda, where a correctly-signed notification carrying a decimal STRING amount verified, split the invoice number, coerced the amount and flipped `paid`. What is missing is one of DOKU's own bodies, not the machinery that reads it | **ONE DELIVERED NOTIFICATION, sandbox or production.** `DOKU_CAPTURE` is already set in Preview and stays, so the first one that arrives is captured without any further setup. It becomes `tests/fixtures/doku-notification.sandbox.json` and the §4 signature tests re-point at it. **CHECK THIS BEFORE THE PRODUCTION FLIP, not after:** a first real buyer is a bad place to discover that a field moved |
| **THE `DOKU_CAPTURE` DEBUG LOG ON `/api/doku/notify` - added 2026-09-21, REMOVE IT ONCE THE FIXTURE LANDS** | A flag-gated `console.log` of the four headers and the raw body, fenced three ways: an explicit `DOKU_CAPTURE`, never under `VERCEL_ENV=production`, and only AFTER the HMAC passes. It exists because V-doku.md §5 asks for the raw notification "from the function log" and nothing was putting it there - the last walk could not have produced a fixture even if DOKU had delivered one. **WHAT IS UNGUARDED: nothing today.** All three fences are asserted and each was shown red, including by moving the capture above the verify. What it COSTS is a debug path on the payment route, and the argument for deleting it is simply that a temporary thing with tests around it is the kind that stops being temporary | **AMENDED 2026-09-21, AND IT NOW STAYS.** This row used to close on the fixture landing. Three walks later nothing has been delivered to capture, so the capture is not a loose end waiting on a commit - it is the INSTRUMENT that catches the first notification DOKU ever sends, and removing it would mean rediscovering all of this. Reyner ruled `DOKU_CAPTURE` STAYS SET in Preview. It closes when a delivered notification has been captured AND the fixture is in the repo, in one commit that removes both together |
| **RECLOSE-DEFERRED: A PAYMENT STARTED JUST BEFORE THE FENCE CLOSES IS NOT RECONCILED WHILE IT STAYS CLOSED - ruled 2026-09-23 (Reyner)** | Ruled verbatim (launch cut §0b): *"a payment started just before the fence closes is not reconciled while the fence stays closed. This doesn't affect launch. It goes in the DEFERRED REGISTER until payments can be safely reconciled while closed."* **WHAT IS UNGUARDED: re-closing sales AFTER launch.** The on-load reconcile (`lib/doku/reconcile.js`, #129 and the mirror-reconcile PR) makes no DOKU call unless `paymentsProvider() === 'doku'`, so if `PAYMENTS_PROVIDER` is flipped back to `closed` while a buyer is mid-payment, and DOKU's notification does not arrive (it never has, ticket 1149053), her row stays unpaid and only `npm run doku:status` + a manual settle recovers it. The notify route still settles on key presence (`dokuConfigured()`), so a notification that DOES arrive is unaffected. Launch is unaffected: before launch the fence has never been open in production, so no invoice exists to strand | **Reconcile EXISTING invoices while closed**: let `reconcileWithDoku` ask DOKU when the fence is closed but the keys are present AND the row already carries a DOKU invoice (`${id}.<suffix>`) - asking about money that already moved, never starting a sale - with its own test that a closed fence still makes no call for a row with no invoice. Owner: Reyner rules when a re-close is on the table |
| **FRONT-DOOR ADAPTIVE LAYOUT BEYOND HEADER WIDTH - parked at the launch cut 2026-09-21, recorded with Prompt AA 2026-09-24** | A centred 460px column is the Google-homepage shape AA builds; a wider canvas for laptops is not built. **WHAT IS UNGUARDED: nothing for a customer** - a wider canvas is a taste choice, not a defect. AA §6's one open check (does the 460px header read as intended at 1280, or as a phone column?) is shown to Reyner on the preview as two shots | Reyner picks from the 1280 shots; if the header should widen, `maxWidth: 460` in `SiteHeader.jsx` / `SiteFooter.jsx` becomes ~720 with the content column unchanged - a one-line CSS change, visible, so his |
| **PILAR KONSEPSI SHOWN, NOT EXPLAINED - added 2026-09-24 (round-1 markup R6, Cowork by Reyner's delegation)** | "Pilar Konsepsi shown, not explained; needs a ruled `label_meaning` for 胎元." Both paid PDFs print the conception pillar as a fifth cell on the chart page (AB A11), and since A7 (#126) the appendix omits its row because the cell's `label_meaning` is empty, so a buyer sees the term and nowhere is it explained. Nothing false is printed; it is a gap, not a defect. Parked at the two-round cap, round 2 of #128 | A `label_meaning` for `pilar.conception` ruled by Reyner (content, his register), then the appendix prints it through the existing path with no code change: A7 omits a row only while its meaning is empty. Note the 2026-08-07 ruling that the cell carries no meaning ON PURPOSE (prompt M correction 4) - this row asks him to revisit it, it does not presume the answer |
| **NOTIFICATION URLs ARE PER CHANNEL, AND PRODUCTION WILL NEED ITS OWN - added 2026-09-21** | Not a defect; a fact with teeth. DOKU registers a Notification URL against each PAYMENT CHANNEL, not against the account. The 2026-09-21 walk delivered nothing because the URL sat on the inactive QRIS channel while the payment went through VA BCA - the buyer paid, DOKU recorded SUCCESS, and the row never flipped. **WHAT IS UNGUARDED: the production flip.** "Is the URL registered?" is one question PER CHANNEL, and it can be yes for a channel nobody uses while being no for the one taking money. The failure is silent by construction - nothing errors, and the only symptom is a paid customer with no product | **A check at the production flip**, which is Reyner's step and not a commit: register the URL on the QRIS channel in the PRODUCTION Back Office, then confirm with one real purchase that the row flips. `npm run doku:status` is the instrument if it does not |
| **WRITER MODEL REVIEW, POST-LAUNCH - added 2026-09-25 (Reyner's ruling of 2026-09-24, Prompt AC §C)** | Reyner, verbatim: *"Writer model review, post-launch. Candidates seen 2026-09-24 in AI Studio: gemini-3.5-flash-lite $0.30/$2.50, gemini-3.8-flash $0.75/$3.75 (promo to 2026-12-31); current gemini-3.1-flash-lite $0.25/$1.50 per 1M tokens in/out. Pin exact ids, never -latest aliases. Also consider a cheaper judge after J1 calibration settles."* The prices are Reyner's reading of AI Studio, not re-checked here; the current writer's matches the price table on `feat/voice-v2` (ai.google.dev pricing, read 2026-09-24). Voice v2 round 3 ran on the UNCHANGED writer (`lib/render/config.js` `TIER_MODELS`, `gemini-3.1-flash-lite`) and the unchanged Pro judge (`gemini-3.1-pro-preview`), by the same ruling. **WHAT IS UNGUARDED: nothing for a customer today.** The writer is pinned and serving. What waits is a quality or cost gain, and the judge's cost: in round 2 it ran 15-36x the writer per reading (`docs/qa/2026-09-24-voice-v2-renders-v1-v2.md` on `feat/voice-v2`). On that branch, unmerged, J1 now GATES (STAGE6 1.33.0), so a cheaper judge is no longer a free swap | After launch. A candidate writer is measured the way v2 was: the same subjects through `feat/voice-v2`'s `scripts/qa-voice-v2-renders.mjs`, read as PDFs by Reyner. A candidate JUDGE must first pass that branch's `scripts/calibrate-j1.mjs` (every seed caught in 3 runs, zero clean J1 false positives). Exact model ids only; a `-latest` alias can move under a pinned test |
| ~~**VOICE V2 PARKED; LAUNCH ON v1 - ruled 2026-09-25 (Reyner, launch cut §0c `VOICE-V2-PARKED`)**~~ **SUPERSEDED 2026-09-26: VOICE V2 IS LAUNCH-CRITICAL, AND KATON DOES NOT LAUNCH ON v1.** Superseded by `docs/content/voice-constraint-rulings-2026-09-26.md` STATUS (*"Katon does NOT launch on v1 because the DOKU gate is met"*) and `docs/product/product-boundary-rulings-2026-09-26.md` items 5 and 9 (MVP render path: engine -> Flash-lite writer -> deterministic checks -> serve; no semantic judge in production; no model bake-off). The cells to the right are the 2026-09-25 record, kept unedited; their PARKED status, "J1 as a HARD gate" and "reopens post-launch" are not in force | **WHAT IS PARKED:** the voice-v2 writer (a free writer on short prompts, plus a strict factual reviewer, spec `docs/content/voice-v2-spec-2026-09-24.md`), after its capped third round. **WHAT THE BRANCH HOLDS**, `feat/voice-v2` at `09f1c45`, unmerged and NOT to be deleted: the v2 prompts and the `VOICE=v2` switch (`lib/voice.js`, which returns `v1` whenever `VERCEL_ENV=production`); engine changes E1-E3; D1-D4; the judge (J1-J4) with **J1 as a HARD gate** and its calibration harness (`scripts/calibrate-j1.mjs`, PASS 15/15 seeded, 0 clean false positives, `docs/qa/2026-09-25-voice-v2-j1-calibration.md`); fix (i) archetype brackets from the engine and fix (ii) typography normalisation; STAGE6 `1.26.0`-`1.33.0` (1.31-1.33 are round 3); the round-2 and round-3 render harness and QA docs. **Migration 0011 is NOT applied.** **WHY PARKED** (Cowork's round-3 read, as relayed in the ruling; not re-measured here): v2 mirrors are shorter and drop facts v1 covers; concrete actions 0 in v2 against 1-4 in v1; v2 added "ingatlah"; the pair portraits are unreliable; the judge costs 15-36x the writer per reading (measured in round 2, `docs/qa/2026-09-24-voice-v2-renders-v1-v2.md` on the branch). The one clear v2 win, J1 catching the g4WH4 inversion that D1-D4 served, is kept on the branch rather than lost. **WHAT IS UNGUARDED: nothing for a customer.** Production serves v1: main carries no v2 code at all (`git cat-file -e origin/main:lib/voice.js` fails; `lib/render/prompt.js` loads only `renderer-prompt.txt` and `compat-renderer-prompt.txt`). The g4WH4-type slot-filling error (the Penyeimbang Unsur glossary sentence with an element name inserted, `docs/qa/2026-09-25-voice-v2-round3-renders.md`) is a v2 observation; whether v1 makes it has not been measured | ~~**Reopens with the WRITER MODEL REVIEW row above**, post-launch. Its measurement is the same one v2 needs: the same subjects through the branch's harness, read as PDFs by Reyner, and any judge must first pass `scripts/calibrate-j1.mjs`. Rebase `feat/voice-v2` onto main before any reopening run: it forked before #138 (the "gate is intentionally loose" spec text and the D4 correction) and must carry them~~ **SUPERSEDED 2026-09-26: not reopened post-launch; v2 is the launch voice.** The launch sequence is the one in `docs/content/voice-constraint-rulings-2026-09-26.md` STATUS, and the judge is a development/QA tool only (`docs/product/product-boundary-rulings-2026-09-26.md` item 5) |
| **A SEPARATE PREVIEW DATABASE - added 2026-09-25 (Cowork ruling, rule 9; Reyner may overrule)** | Preview deployments write to the production Supabase project (THE INTERIM REGISTER, "PRODUCTION AND PREVIEW SHARE ONE SUPABASE DATABASE", with the evidence). **WHAT IS UNGUARDED:** (a) any Preview build made while Preview was `mock` still answers `POST /api/mock-pay/<id>`, which flips `paid` on any row, including one created on production (25 live branch aliases plus every such deployment's own URL, probed 2026-09-25 11:47Z); (b) a Preview build with `PAYMENTS_PROVIDER=doku` and the sandbox keys (today `ops/doku-walk` alone) can settle ANY row against a SANDBOX payment, including one created on production: its `/api/pay` writes a sandbox invoice onto the row, and a free simulator payment then settles it through reconcile. Walk 4 did exactly that for its own row. By code reading, not exercised on a production row. It ends when that branch variable is deleted, on flip day before sales reopen (gate i); (c) test rows sit among real ones with no column to tell them apart, so every purchase count needs the KNOWN TEST ROWS list. **What limits it today:** the ids are 21-character CSPRNG tokens and not enumerable (rule 19), and the general Preview `PAYMENTS_PROVIDER` is `closed` since 2026-09-25, so NEW builds have no mock door | **Trigger: Prompt U (the Render move) or the first promotion beyond Reyner's own network, WHICHEVER COMES FIRST** (the chain order in "THE CHAIN AFTER IT, CORRECTED 2026-09-14 NIGHT" below). Then: a second Supabase project for Preview; migrations 0001-0010 applied there by hand, before the Preview env points at it (REPO CONVENTIONS); Preview `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` re-pointed; the pre-change `mock` deployments closed by Reyner (deleted, or behind Vercel deployment protection). Owner: Reyner (Supabase, Vercel) |

**`p1_stem_relation` IS NOT A REQUIRED POINT, logged 2026-09-13 with the round-2 close.** A model
that skips P1 entirely passes coverage - nothing in the pair guard makes the stem-relation block
mandatory, so the 38-run example duplication above is measured on a block the gate does not even
require. This is a note and not a register row because no one has yet argued it should be required;
it is written down so the next session reads it as a known property rather than finding it as a hole.
...
- `product/product-boundary-rulings-2026-09-26.md` . free Mirror stays whole, one canonical reading,
                                         Compat CTA at the curiosity moment, and the MVP render path
                                         (Flash-lite writer, deterministic checks, no production judge).
...
| **Y-4 TOOK THREE PAGES OUT OF THE COMPAT PDF AND ONE OUT OF THE MIRROR** | compat Y-1 **11 -> 8 pages** (appendix 6..8), mirror chart 1 **8 -> 7** (appendix 6..7) · fixed point **1 rebuild -> 0** on both · collisions **0** on both · compat file **33 KB -> 20 KB** | **09-14** | The saving is the page-number cross-references going (R6): the "Yang ada di baganmu" list was a full column on every chart page and the compat document had THREE of them. The appendix got LONGER in rows and shorter in height by becoming a two-column table (R7). **PAGE COUNT IS A CONSEQUENCE AND Y-4 SAYS TO ASSERT NOTHING ON IT** - recorded so a later change that doubles it is visible, never as a target. The prompt's targets (1 page of appendix for the mirror, 1-2 for a pair) are MISSED by one each: the mirror's appendix is 2 pages and the compat's is 3, because R8 prints every ruled cell WHOLE and the compat legend carries 34 entries across two charts. Closing that needs either shorter cells (Reyner's words) or a smaller type scale (a design pass Y-4 excludes), so it is flagged rather than traded off. Measured on the deterministic floor, so a model-rendered reading pages differently. |
| **THE COMPAT PDF IS 10-11 PAGES, AND THE FIXED POINT CONVERGES IN ONE REBUILD** | Y-1 fixture pair **11 pages**, appendix starts p9, 32 anchors, 28 referenced, 1 rebuild · 2x6 **10 pages**, appendix starts p8, 35 anchors, 32 referenced, 1 rebuild | **09-14** | A CONSEQUENCE, NEVER A TARGET. Nothing is tuned to a page count and nothing should be; the number is here so a later change that doubles it is visible. Both pairs pass all three verifies through `buildPairPdf`, which is the claim that matters - the map printed equals the map the bytes record, no reference points outside the appendix, and every reference row is drawn on its own page. Measured on the deterministic FLOOR, which is byte-stable, so this row cannot move because a model wrote differently today; a model-rendered reading is longer prose and will page differently. Command: `buildPairPdf` over the two fixture pairs, reported by `tests/pdf-pair-document.spec.mjs`. |
| **ROUND 2 WON THE VOICE AND LOST THE FLOOR, AND THE FLOOR CAUSE IS ONE CHECK ON ONE RULED MOVE** | floor **5/20** (4/10 then 1/10), against 3/10 round 1 and 1/10 baseline, every one `stage6_budget_spent` · `pair.penutup_register` **2/20** · `pair.direction_resolved` **3/20** · `p0_model_wrote_anyway` **0/20** · ruled-seed transcriptions **13/42 -> 9/42** against a target of 3/42 · worked-example nine-word runs **38**, target 0 | **09-13** | Artifact `docs/qa/2026-09-13-compat-three-pair-walk-v2.md`; draws in `...-round2b-draw1.md` and `-draw2.md`. **The three visible floor literals are all `style.hedge_construction` on the P2 reframe** (`:162`, `:219-220`, `:248`: *"bukan vonis ketidakcocokan, melainkan ti"*, *"Ini bukan penentu kegagalan, melainkan tanda"*), which is the prompt's own mandated `p2_reframe` move said in the banned shape - **a ruled instruction and a ruled check pointed at each other**, not a round-2 regression. Reyner ruled the construction VALID and the gate the cause on **2026-09-13** (R2, recorded verbatim in `lib/validate/blocklist.json#style._hedge_construction_pair` and in `docs/prompts/Z-close-2026-09-13.md`); the pair-only exemption is `STAGE6_VERSION` 1.25.0. **13/42 REPRODUCED on the staged tree** with `node scripts/count-seed-runs.mjs --falsify`, whose control on the example text returned 54 runs, so the counter is shown able to fire before its figure is quoted. `style.hedging` on `cenderung` is NOT covered by this exemption and stays a logged observation (Z-close section 5). |
...
## DONE 2026-09-18 — Prompt V-0: the Xendit adapter is deleted. PAYMENTS_PROVIDER is `mock | closed`

Reyner ruled the exit the priority ("fully leave Xendit asap, make it priority"), which pulls adapter
deletion out of Prompt V's commit 5 and lands it on its own. **V now ADDS a DOKU adapter to a two-value
fence instead of swapping one adapter for another**, so the exit stops depending on DOKU's pace.

**A READER SEES NOTHING.** Sales were already closed in production (`PAYMENTS_PROVIDER` unset ->
`closed`). What this buys is that the exit is DONE rather than pending, and that a stale
`PAYMENTS_PROVIDER=xendit` in any environment can never again point at a terminated account.

**GONE:** `lib/xendit.js`, the whole `app/api/webhook/` directory (`xendit/` was the only route in
it), the `xendit` provider value, the fence's `NODE_ENV` dev bypass, both `XENDIT_*` key checks,
`devBypassAllowed()`, the pay route's real-provider branch with its redirect-URL block and its
`not_configured` dev fallback, and the route's now-unreachable generic `payment_not_configured`
refusal. `.env.example` documents `PAYMENTS_PROVIDER` for the first time.

**THE RED LINE, from the commit that landed the instruments alone against the UNCHANGED fence:**

```
  A STALE PAYMENTS_PROVIDER=xendit MEANS CLOSED
    AssertionError: Expected values to be strictly equal: 'xendit' !== 'closed'
  NO XENDIT IN THE PAYMENT PATH
    AssertionError: no file that decides or records a payment may name the adapter
  THE ADAPTER AND ITS WEBHOOK ARE GONE, and nothing imports them
    AssertionError: the adapter is deleted, not merely unreferenced -- true !== false
  THE PAY ROUTE ANSWERS payment_closed, not a misconfiguration
    AssertionError: no provider call survives in the route -- true !== false

  ℹ tests 12  ℹ pass 8  ℹ fail 4
```

**A REAL BEHAVIOUR CHANGE, NAMED RATHER THAN BURIED:** development with no provider used to return
`null` and permit. It **refuses** now. The bypass existed to stand in for an absent adapter and
`mock` is the supported way to walk the flow, but nothing else in this PR changes what any
environment does, so this one is asserted in two places.

### THE PROMPT'S GREP WAS SCOPED WRONG, AND RUNNING THINGS IS WHAT CAUGHT IT

The prompt stated `devBypassAllowed()`'s "only callers are the two files being deleted/edited here"
and listed them. There were **four**: `tests/compat-surface.spec.mjs` and `scripts/forge-tests.mjs`
also import it. The prompt's own grep covered `lib app components tests` and **`scripts/` was never
in it**. Nothing in the prompt was wrong about what it looked at; the scope was.

Three dependencies it did not list, each found by a red rather than by a re-read:

| What | Why its premise is gone | What replaced it |
|---|---|---|
| `tests/compat-surface.spec.mjs`: `IN PRODUCTION WITH NO XENDIT KEY` and `THE WEBHOOK TOKEN IS THE SECOND GATE` | Both asserted `paymentFenceReason()` returning the `xendit_*` strings | Deleted. The dated diagnosis of Reyner's 2026-09-08 preview failure is KEPT as a comment; the fence's current answer for the same submit is still asserted directly below it |
| `tests/compat-surface.spec.mjs`: `A PAIR GETS BOTH REDIRECT URLS` | Read `successRedirectUrl: pairUrl(id, ...)` out of a branch that no longer exists | The half with a subject survives: `pairUrl` and `readingUrl` stay distinct builders, plus an assertion that the route imports neither. **The X-b1 requirement moves to `docs/NEXT.md`'s Prompt V line** - a test red for the whole of V-0's life teaches the next session to delete it rather than satisfy it |
| `scripts/forge-tests.mjs`: four fence cases and two `--live` webhook forgery checks | The four named `PAYMENTS_PROVIDER=xendit`; the two POSTed at the deleted `/api/webhook/xendit` | Two cases that still have a subject - unset means closed, stale `xendit` means closed - plus the dev-bypass change. **The first forgery check would have PASSED a 404** had its accepted set been one value wider, which is the shape CLAUDE.md's card-check convention is about |

### THE ACCEPTANCE GREP DOES NOT RETURN ZERO, AND IT SHOULD NOT

The prompt's acceptance asked for `grep -rn -i xendit ... lib app components tests .env.example` ->
zero hits. **That is incompatible with the same prompt's "does not touch pricing, copy, or any
reader-visible surface", and the copy instruction is the one that wins.** What survives, deliberately:

- **Reader-visible Indonesian in `lib/site/copy.js`** (~6 strings: the privacy policy, the terms, the
  `/harga` payment line, the refund page). They name the processor that **actually handled** the
  payments those documents describe, so they are not false. Rewriting them is a COPY change, copy is
  Reyner's alone (rule 20), and the replacement sentence has to name DOKU - **which does not exist
  yet**. Prompt V's work, flagged to Reyner, not guessed here.
- **Dated history in comments**: why `/tentang` and `/harga` exist, why the footer and static pages
  are server components, `lib/site/entity.js`'s two rulings, the X-b1 record, the
  `NEXT_PUBLIC_FREE_FULL_READING` scar in `tests/deliver-route.spec.mjs`. Commit 3's own instruction
  is "reword ONLY sentences that describe the CURRENT mechanism"; these record what was true when
  written and the merchant review genuinely happened.

So the guarded proposition is narrower and checkable: **no file that decides or records a payment
names the adapter, and nothing anywhere imports the module.** A repo-wide ban would have carried a
seven-file exclusion list, and a check that is mostly exclusions is a check written from the
requirement it documents rather than from the defect it catches.

```
$ npm run -s test:payments-provider    ℹ tests 12  ℹ pass 12  ℹ fail 0
$ npm run -s test:compat-surface       ℹ tests 19  ℹ pass 19  ℹ fail 0
$ npm run -s report:forge              PASS — 22 passed, 0 failed
```

**`INVOICE_DESCRIPTION` IS KEPT UNREFERENCED WITH AN EXPLICIT `eslint-disable`.** Its block records
three supersessions of that one string and states that guessing it has a measured track record of
being wrong; handing Prompt V a blank line there is the more expensive mistake.
`scripts/check-unruled-copy.mjs` does not protect it - that gate scans the registered `COPY_BANKS`,
so the `compat` sentinel is caught through `COMPAT_COPY`.

**WHAT THIS PR DOES NOT DO, and the deadline is on this half.** It does not close the Xendit account,
withdraw the balance, or unset the keys in Vercel. Those are **Reyner's**, the 1 October fee is on the
ACCOUNT rather than on the code, and the INTERIM REGISTER row stays **OPEN** until his written
closure confirmation is in hand. A merged PR that reads like it closed the account is exactly how an
October charge arrives against a repo everyone believes is clear.
