...
| **Compatibility, Rp 39.000 launch / 49.000 list. PURCHASABLE, AND A READER CAN REACH IT** (2026-09-08, X-b3) | **What she gets:** a front-door card at `/kompatibilitas`, a static pre-payment page (title, lead, five inclusions, price, two births, an email), QRIS checkout, then the report at `/kompatibilitas/<id>` - the prose blocks with the P4 badge and the P5 quadrant as eyebrow'd labels, and the URL on screen as her only way back. **NOTHING COMPUTED IS SHOWN BEFORE PAYMENT** - no chart, no archetype, no tease; the one pre-payment computation is the season gate, which is a fact about a DATE and is asked so nobody buys a reading of the wrong month pillar. **IDENTITY IS THE EMAIL PLUS THE URL**: no account, no password, and in v1 nothing is sent - the email is stored for access and recovery (ruled 2026-09-08). **THE FLOOR IS PRESENTED IDENTICALLY to a Gemini render** - it is Reyner's own ruled glossary prose (rule 15), so `served_from` is not read by the page and a test holds it that way. No card, no PDF, no share in v1: a compat card would put two people's archetypes on a shareable image and person B consented to nothing | `app/kompatibilitas/` -> `components/Pasangan.jsx` / `PasanganReport.jsx` -> `POST /api/pair` -> `POST /api/pay/<id>` -> the verified webhook -> `GET /api/pair/<id>[/reading]` | ~~not purchasable~~ **STRUCK 2026-09-08.** `compat` is in `SELLABLE_SKUS` since `a530d56`; `paid` flips in the verified webhook alone (rule 18). **EVERY READER-FACING STRING IS A LIVE `@@UNRULED@@` PLACEHOLDER - 28 of them - so a PRODUCTION BUILD REFUSES.** That is the intended state: the preview passes so Reyner can walk the surface on a phone in order to rule it. The Upcoming tap that used to measure demand for this product is DELETED; demand is measured by purchases from here on |
