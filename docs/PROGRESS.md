...
| **Card A / Card B** (built 2026-08-13, two review passes same day) | **Nothing. Wired to no route and no component.** Renders only into `reports/card-preview.html` via `npm run preview:cards` | `lib/card/` + `components/cards/Card.js`, from `glossary.json` + Stage 3 facts | **NOT SHIPPABLE YET, for four reasons:** five of ten colour tokens are unapproved; **two tokens cannot reach WCAG AA at any opacity** (`丙` Matahari is LOCKED, so its fix is Reyner's); the archetype names disagree with the reading's (see below); and nothing loads Archivo, its ruled typeface, outside the preview page |
...
| **Two colour tokens cannot reach WCAG AA** | `丙` Matahari ink 3.04 and `戊` Gunung ink 4.21 against their own fields. Not an opacity problem: opacity 1 is already the maximum, so no role change reaches 4.5. Every other token clears AA and the card's whole text layer moved to `ink` to get there. `npm run audit:card-contrast` | **A TOKEN CHANGE, and Matahari is LOCKED so it is Reyner's call.** Minimums measured 2026-08-13: Matahari clears at 4.53 by darkening the field to `#CC3F0E` with the near-white ink kept, or at 4.51 by keeping the vivid field and flipping to a dark ink `#4A1705` (which makes it a light-field token like Permata). Gunung is only PROPOSED, so it moves with the token ruling: `#896B3D` clears at 4.53, a 4.5% darkening that is barely visible |
