...
| **RULE 23 ENFORCED - and it COSTS ONE CHART ITS READING, which is the number that needs pricing** | floor rate **0/4 -> 2/4** on single renders · attributable to enforcement **1 of 4** (chart 1) · the other floors anyway · `brackets.unbracketed` fires on **7 of 9 attempts** across the run · bracket rate before enforcement **19/21** | **08-21** | `STAGE6_VERSION` 1.11.0 -> 1.12.0; `brackets.unbracketed` and `brackets.mismatch` go `flag` -> `soft`. Ruled by Reyner on the sentences: `Kamu adalah Matahari (The Sun) yang Lemah` over `Kamu adalah Api Matahari yang Lemah`. **THE COUNTERFACTUAL IS COMPUTED, NOT GUESSED**, off the recorded per-attempt checks in `docs/qa/2026-08-21-renders-rule23-enforced.md`: chart 5 and chart 13 are served at attempt 2 either way; **chart 1's attempt 1 failed on `brackets.unbracketed` TWICE AND NOTHING ELSE, so before this change it was served first try and now it floors**; fresh-1996 fails non-bracket checks at all three attempts and floors regardless. So the honest split of a 2-chart climb is **one attributable, one not**. **THE COUNTERFACTUAL FOR ATTEMPTS 2+ IS A BAND, NOT A POINT**, and for the reason already in this ledger (row 509, and COWORK-BRIEF row 37): `stricterDirective` conditions the next prompt, so removing a finding from attempt 1 changes what attempt 2 generates. Chart 1's figure is FIRM because it is attempt 1, which nothing upstream conditioned. **AN INTERACTION WORTH NAMING:** both floored charts have an attempt that failed on `opening.archetype_missing` alone - the regeneration sent to fix brackets rewrote the opening and dropped the archetype, so commit 1's check and this one can chase each other inside one budget. **A HEADING IS NOT A FIRST MENTION, and finding that out is why the check reads prose only.** Run over the whole reading, the first occurrence of `Aspek Pengatur` is the module-assembly floor's own bare HEADING, one line above the correctly bracketed sentence - so bracket-once rejected the always-available floor on every fixture chart until `renderedProse` was introduced. Rule 17 does not allow that, `fallback.js` already says a heading is a separate visual element, and `opening.js` excludes headings for the same reason. **REPORTING FIRST DID NOT CATCH IT**, because a `flag` fails nothing and the 19/21 figure was measured off artifact prose that never contained headings: the measurement and the gate disagreed about the haystack. Measure with the instrument the gate uses |
