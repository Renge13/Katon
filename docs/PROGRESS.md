...
| Compatibility | **A NAME, A PRICE AND A TAP. Not purchasable, and nothing is delivered.** It appears in the upcoming block below the Artifact decision. Tapping it records interest and nothing else | `components/Funnel.jsx#Upcoming` -> `POST /api/mirror/[token]/event` -> `product_interest` | `compat` is priced (**49.000 list / 39.000 launch**, ruled 2026-08-29 - this row said 45.000/29.000 until now, which was the pre-ruling ladder) and **absent from `SELLABLE_SKUS`**, so `/api/pay` 400s. Verified by hand 2026-08-31 against the running dev server: `POST /api/pay/<token>` with `{"sku":"compat"}` -> `400 {"error":"sku must be one of: artifact"}`. ~~**The Upcoming tap is scheduled for deletion in X-b3 when the standalone entry point ships**~~ **DELETED 2026-09-08, X-b3 commit 3.** The row, its tap and its two ruled strings are gone from `Upcoming`, which advertises what is NOT for sale - and compat now has a route, a price and a checkout. **THE DEMAND INSTRUMENT CHANGES WITH IT: demand for compat is measured by PURCHASES from here on, not by taps.** That is a strictly better instrument and the swap is worth stating rather than leaving implied, because the tap produced a number nobody can reproduce any more: `product_interest` rows for `compat` stop accruing at this commit, so any later read of that table is a count of a window that closed, and it must be dated. `annual` keeps its tap and its `Belum tersedia`. |
