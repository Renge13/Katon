...
| 47 | **CLAUDE CODE (2026-09-08). A GATE THAT ENUMERATES ITS SUBJECTS BY NAME IS BLIND TO THE NEXT SUBJECT — three instances in three days, each fixed by widening the list, which is what kept the defect alive.** `scripts/check-unruled-copy.mjs` decided WHAT to scan from a hand-written `BANKS` object. **(a) 2026-09-07:** `COMPAT_COPY` shipped with a deliberate `@@UNRULED@@` invoice description; the gate scanned `UPCOMING_COPY` alone and printed `OK No unruled copy in 1 bank(s)` beside a live placeholder on a payment path. **(b) 2026-09-08:** `GLOSSARY.kompatibilitas` landed with 59 placeholders; the gate printed `OK ... 2 bank(s)` and exited 0. **(c) and underneath both, row 46:** the gate had never executed on Vercel at all. Each was found by a human noticing, never by the gate, and **each fix added one line to the list** — so the third instance was committed by the same session that had just fixed the second. The oracle in `tests/unruled-copy.spec.mjs` carried its own copy of the same list and was widened alongside it, twice, which meant the check and its independent cross-check went blind together and agreed | **THE FIX IS INVERSION, NOT A FOURTH WIDENING, and the distinction from row 46 is which END of the guard was unwatched.** Row 46 is a gate nothing invoked — a missing TRIGGER. This is a gate invoked correctly on the wrong SUBJECTS: it ran, on every build, and reported success about banks nobody had told it existed. **A list of what to check is a second source of truth for what exists**, and it goes stale in the one direction that reads as a pass — a bank added is invisible, a bank removed is a loud crash. That asymmetry is why three rounds of widening felt like three fixes. **THE RULE: a gate discovers its subjects, it does not remember them.** Banks now `register()` into `COPY_BANKS` at their definition sites; `lib/site/unruledScan.js` is the one place a scannable source is named; the gate and its oracle both enumerate that and neither names a bank. The assertion that makes it structural rather than a tidier list: the spec reflects over `import * as copy from lib/site/copy.js` and fails if any `*_COPY` export is missing from the registry — shown red by exporting an unregistered `TEST_COPY` (`TEST_COPY is exported but never register()ed`). **AND IT IMMEDIATELY CAUGHT WHAT THE LIST HAD ALWAYS MISSED:** `SITE_COPY`, the largest bank in the repo, had never been scanned by the gate in its entire life. It carries no sentinel today, which is luck rather than design |
| 48 | **CLAUDE CODE (2026-09-08). A UI TEST AGAINST A FIXTURE THE DATA SOURCE NEVER PRODUCES, AND A BROWSER CHECK THAT STOPPED AT THE PAYWALL - both green while a PAYING CUSTOMER received a blank page.** Reyner paid Rp 39.000 on production for pair `g4WH4_9QbCrCj3Gha934q` and got six headings, no paragraphs, the raw keys `contrasting` and `q4` as text, and only the penutup as prose. **Two independent defects, both shipped in #105, both invisible to a 58-test suite.** (a) `components/ProseBlocks.jsx` read `block.paragraphs`; the render contract is `{fact_ids, heading, text}` and `lib/pair/serveReading.js` sends exactly that. The MIRROR works only because `lib/mirror/view.js:206` runs `splitParagraphs` on its way out - the adapter existed on one of two paths and the component assumed every caller had applied it. (b) the payload sent `semanticJson.core.pattern`/`.quadrant` and the component labelled two blocks from them believing they were glossary `name_id`s. **Every fixture in `tests/pair-report-render` predecessors was hand-built from the shape the test expected**, so the serve payload and the component were each checked against an idea of the other and never against each other | **THE FIXTURE WAS THE ASSUMPTION UNDER TEST, WRITTEN DOWN TWICE.** That is the distinction from row 47 and from row 46. Row 46 is a gate nothing invoked - a missing trigger. Row 47 is a gate invoked on the wrong subjects - it ran and reported on banks nobody told it about. **This is a check whose INPUT was manufactured by the same belief it was meant to test**, so it could only ever confirm. A hand-made fixture encodes what the author thinks the producer emits; when that is the very thing in doubt, the test is a mirror. **AND THE BROWSER CHECK IS THE OTHER HALF, and it is mine:** #105 was walked in a real browser at 375px and the walk stopped at the form, because going further needed a payment. The one screen never rendered by anything - test or human - was the one behind the paywall, and it was the one that shipped broken. **THE RULE: a fixture for a boundary is CAPTURED from the producer, never authored; and a surface no walk can reach needs a fixture from the real thing precisely because no walk can reach it.** `tests/fixtures/pair-reading-g4WH4.json` is the verbatim production response, and the assertion that would have caught both is one line: every block renders at least one paragraph. It failed with **"expected at least 7 non-empty paragraphs, got 1"** - the penutup, read from a different field. The free-walk path (`PAYMENTS_PROVIDER=mock`, `docs/ops/paid-flow-walk.md`) exists so the next paid surface can be walked before a customer walks it |
| 49 | **COWORK (2026-09-14). Prompt Y-3's page list said "Colophon - the mirror's, unchanged". THE MIRROR HAS NO COLOPHON.** Reyner killed that page on 2026-08-22 ("ending a paid Rp 19.000 personal reading with copy-pasted Terms of Service text kills the product experience right at the finish line") and the disclaimer moved to the foot of the chart page at 8pt as `chartPageFoot`. Caught by Claude Code while composing the compat document, from `lib/pdf/document.js`'s own header - which says FOUR sections, not five, and names the ruling. Implemented as the thing the prompt was pointing AT: the disclaimer and provenance ride the foot of the compat facts page | **A PROMPT NAMING A COMPONENT THAT A RULING DELETED THIRTEEN MONTHS-WORTH-OF-COMMITS EARLIER.** The prompt's own header states the precedence - a ruling recorded in the repo beats the prompt, always - so the recovery was cheap and is not the point. The point is the SHAPE: the section list was written from the mirror document's STRUCTURE as remembered, not as read, and the one item that had been removed by a ruling is exactly the one that survived in memory. A prompt enumerating an existing artifact's parts should be written from that artifact, and the anchors Y-3 DID carry (`lib/pdf/build.js:76`, `handlers.js:179`, `document.js:361`) were all correct - the difference is that those were greps and this was a recollection |
| 50 | **CLAUDE CODE (2026-09-14). THREE INSTRUMENTS WRONG IN ONE COMMIT, AND TWO OF THEM PASSED.** Building the compat PDF: (a) a glyph check compared CHARACTERS against `drawnCodePoints`' Set of CODE POINTS and reported all twelve pillar characters missing - on the MIRROR too, which is what showed it was the instrument; (b) two rate-limit assertions passed while asserting nothing, because `deliver_pdf` declares only an `ip` dimension, `consume` skips a dimension whose value is absent, and a bare `new Request(...)` has no `x-forwarded-for` - so one expected a 429 it never got and the other "proved" an unpaid caller could not spend a budget nobody was spending; (c) a "the PDF authors nothing" assertion passed with a block DELETED, because the floor assembles its blocks out of the glossary `label_meaning`s and the facts table prints those same ruled meanings, so the text was still in the document on another page. All three found by deliberately breaking the thing each was meant to catch | **THE FAILING-DIRECTION ONE IS THE CHEAP ONE AND IT IS NOT THE LESSON.** (a) is loud: it fails, you look, you fix it. (b) and (c) are the expensive shape - green, specific-sounding, and satisfied by something other than the property they name. Both were caught by the same move and only by it: point the instrument at a deliberately broken input and require a red. (c) also names a rule worth keeping: **when a fixture makes two different things equal, an assertion cannot tell them apart.** The floor makes the reading prose and the table content identical; in production they differ, and the PDF never serves a floor at all - so the duplication was a property of the FIXTURE, and an assertion a fixture quirk can satisfy is not an assertion |
