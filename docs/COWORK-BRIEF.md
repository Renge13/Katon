...
| 39 | **UNATTRIBUTED (self-reported by Claude Code, 2026-08-19).** Reported that pooling the brass-text fallback made **"Card A pay for Card B's finish"** — that 甲 丁 戊 壬 lost brass on the free card to solve a sheen problem the free card does not have. **Card A draws no brass text at all.** All three roles reading `brassText` are Card B only, following from the 08-14 ruling that Card A carries NO FINISH. Reyner ruled the per-card split on that premise, and the premise did not exist | **A CLAIM ABOUT WHAT A COMPONENT DRAWS, MADE WITHOUT GREPPING WHERE THE ROLE IS DRAWN.** CLAUDE.md's own convention covers it exactly: a claim about the code without its grep is a memory, not a fact — and this one was worse than a memory, because it was an inference from a colour table to a rendered surface, with the two files open. **It cost a ruling.** One `grep -n "roleStyle('nameId'\|badgeLabelFoil\|pillarLabelDay"` would have settled it in a second and was run only when a test written to confirm the story FAILED. **Same family as error 32** (a change no existing check could see) arriving from the other side: here the check existed, was written last, and was the only thing that caught it. The split is kept on a rebuilt argument, with the correction in the docblock rather than only in chat, and with a test pinning Card A's zero-brass fact so the guard cannot rot |
