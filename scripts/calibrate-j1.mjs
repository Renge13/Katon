...
// per arm.
//   ... --arm A=<judge module> --arm B=<judge module> [--model <id>]
// scores two judge rubrics on the same cases in one run (AD amendment 2, item 2);
// see ARMS below.
//
...
import { JUDGE_MODEL } from '../lib/validate/judge.js';
...
// The two scene-fail groups amendment 2 scores separately.
const SCENE_GROUP = {
  'fail-lastyear': 'past', 'fail-college': 'past', 'fail-lastmonth': 'past',
  'fail-hourwood': 'chart', 'fail-monthclash': 'chart',
};
const SCENES = [
...
const CASES = [
  ...
  ...SCENES.map(([id, set, sampleCase, anchor, sentence, needle]) => {
    ...
    return {
      id, set, needle, payload: base.payload, group: SCENE_GROUP[id] ?? null,
...
// ── ARMS (Prompt AD amendment 2, item 2, 2026-09-26) ───────
// `--arm NAME=PATH` (repeatable) judges every case with the judge MODULE at PATH,
// on the SAME cases and in the same run, interleaved per case so drift in the
// provider hits both arms alike. The model is forced to --model (default: this
// tree's JUDGE_MODEL), so two arms differ in their rubric only. With no --arm, the
// one arm is this tree's lib/validate/judge.js. Each module's sha256 is recorded.
const { createHash } = await import('node:crypto');
const { pathToFileURL } = await import('node:url');
const armSpecs = process.argv.flatMap((a, i) => (a === '--arm' ? [process.argv[i + 1]] : []));
const ARMS = [];
for (const spec of armSpecs.length ? armSpecs : [`A=${new URL('../lib/validate/judge.js', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/u, '$1')}`]) {
  const [name, file] = [spec.slice(0, spec.indexOf('=')), spec.slice(spec.indexOf('=') + 1)];
  const mod = await import(pathToFileURL(file).href);
  ARMS.push({
    name, file, judge: mod.judgeRendering, prompt_sha256: createHash('sha256').update(mod.JUDGE_PROMPT).digest('hex'),
    module_sha256: createHash('sha256').update(fs.readFileSync(file)).digest('hex'),
  });
}
const MODEL = arg('model', JUDGE_MODEL);
console.log(`model ${MODEL}; arms: ${ARMS.map((a) => `${a.name} (prompt ${a.prompt_sha256.slice(0, 12)}, ${a.file})`).join('; ')}`);
...
for (let run = 1; run <= RUNS; run += 1) {
  for (const c of CASES) {
    for (const arm of ARMS) {
      const out = await arm.judge(c.rendered, c.payload, { model: MODEL });
      const u = out.usage || {};
      ...
      spend[arm.name] = (spend[arm.name] || 0) + cost;
      ...
      results.push({
        arm: arm.name, id: c.id, set: c.set, group: c.group ?? null, run, caught, j1, j1_other: j1Other,
        other_findings: out.findings.filter((f) => f.class !== 'J1'),
        malformed: out.malformed, usage: u, cost_usd: cost, model: out.model,
      });
      console.log(`arm ${arm.name} run ${run} ${c.id.padEnd(16)} J1 ${j1.length}${NEEDLED.has(c.set) ? ` caught ${caught}` : ''} other-J1 ${j1Other.length} (other ${out.findings.length - j1.length}, malformed ${out.malformed.length}) $${cost.toFixed(5)}`);
    }
  }
}
...
const verdicts = {};
for (const arm of ARMS) {
  const R = results.filter((r) => r.arm === arm.name);
  console.log(`\n==================== ARM ${arm.name} ====================`);
  console.log(`| case | set | ${[1, 2, 3].slice(0, RUNS).map((r) => `run ${r}`).join(' | ')} |`);
  for (const c of CASES) {
    const cells = [1, 2, 3].slice(0, RUNS).map((run) => {
      const r = R.find((x) => x.id === c.id && x.run === run);
      if (NEEDLED.has(c.set)) return r.caught ? 'caught' : `MISSED${r.other_findings.length ? ` (${r.other_findings.map((f) => f.class).join(',')})` : ''}`;
      return r.j1.length ? `FP x${r.j1.length}` : 'clean';
    });
    console.log(`| ${c.id} | ${c.set}${c.group ? `/${c.group}` : ''} | ${cells.join(' | ')} |`);
  }
  const missesIn = (pred) => R.filter((r) => pred(r) && !r.caught);
  const fpsIn = (set) => R.filter((r) => r.set === set).flatMap((r) => r.j1.map((f) => ({ case: r.id, run: r.run, ...f })));
  const count = (pred) => R.filter(pred).length;
  const isSet = (s) => (r) => r.set === s;
  const isGroup = (g) => (r) => r.set === 'scene-fail' && r.group === g;
  const misses = [...missesIn(isSet('seeded')), ...missesIn(isSet('scene-fail'))];
  ...
  const needledOther = R.filter((r) => NEEDLED.has(r.set)).flatMap((r) => r.j1_other.map((f) => ({ case: r.id, run: r.run, ...f })));
  ...
  console.log('\nNON-J1 FINDINGS (advisory, not scored):');
  for (const r of R) for (const f of r.other_findings) console.log(`- [${r.id} run ${r.run}] ${f.class}\n${quote(f)}`);

  const past = [count(isGroup('past')) - missesIn(isGroup('past')).length, count(isGroup('past'))];
  const chart = [count(isGroup('chart')) - missesIn(isGroup('chart')).length, count(isGroup('chart'))];
  const fpScene = R.filter((r) => r.set === 'scene-pass' && r.j1.length > 0).length;
  const seeded = [count(isSet('seeded')) - missesIn(isSet('seeded')).length, count(isSet('seeded'))];
  console.log('\nCONFUSION MATRIX (per judge call):');
  console.log('| | J1 on the needle | no J1 on the needle |');
  console.log(`| old seeded | TP ${seeded[0]} | FN ${seeded[1] - seeded[0]} |`);
  console.log(`| scene-fail, past events | TP ${past[0]} | FN ${past[1] - past[0]} |`);
  console.log(`| scene-fail, invented chart facts | TP ${chart[0]} | FN ${chart[1] - chart[0]} |`);
  console.log('| | any J1 | no J1 |');
  console.log(`| clean | FP ${R.filter((r) => r.set === 'clean' && r.j1.length).length} | TN ${R.filter((r) => r.set === 'clean' && !r.j1.length).length} |`);
  console.log(`| scene-pass | FP ${fpScene} | TN ${count(isSet('scene-pass')) - fpScene} |`);
  const pass = misses.length === 0 && fps.length === 0 && RUNS >= 3;
  verdicts[arm.name] = pass;
  console.log(`SPEND $${(spend[arm.name] || 0).toFixed(4)} over ${R.length} judge calls`);
  console.log(`ARM ${arm.name}: J1 CALIBRATION ${pass ? 'PASS' : 'FAIL'}`);
}
const total = Object.values(spend).reduce((a, b) => a + b, 0);
console.log(`\nTOTAL SPEND $${total.toFixed(4)} over ${results.length} judge calls; ${Object.entries(verdicts).map(([a, v]) => `${a} ${v ? 'PASS' : 'FAIL'}`).join(', ')}`);
if (OUT) {
  fs.writeFileSync(OUT, JSON.stringify({
    model: MODEL,
    arms: ARMS.map(({ name, file, prompt_sha256, module_sha256 }) => ({ name, file, prompt_sha256, module_sha256, pass: verdicts[name], spend: spend[name] })),
    cases: CASES.map(({ id, set, group, needle, planted }) => ({ id, set, group, needle, planted })),
    results, spend: total,
  }, null, 2));
}
